...
| 契约 | 涉及侧 | 文件 |
|---|---|---|
| relay HTTP API（端点/形状/CORS/安全边界） | `upstream/web/server.mjs` ↔ `dsh-tab-genoffice` 面板 / 各 app `web-bridge.ts` | [relay-api.md](./relay-api.md) |
| **控制契约（DSH 控制编辑，原生集成）**：SSE 下行 / notify 上行 / tool / context / export / `POST /api/file` 写回 / docId 规则 / 工具名集合 | `upstream/web/server.mjs` 控制面 ↔ `apps/{docs,markdown}/src/renderer/control.ts` ↔ `tab-genoffice/src/host/*` ↔ `scripts/dev.mjs` smoke | [control-api.md](./control-api.md) |
...
- `../upstream/web/server.mjs` — relay 实现（权威；控制面段为 control-api.md 的权威实现）
- `../upstream/web/open.mjs` — CLI 注入（inject 形态的生产者）
- `../upstream/apps/{docs,markdown}/src/renderer/web-bridge.ts` — `open=` 形态的消费方
- `../upstream/apps/{docs,markdown}/src/renderer/control.ts` — 控制适配器（control-api.md 消费方/执行器）
