...
exports.classifyControlError = classifyControlError;
function triple(zh, upstream, next) {
    var src = upstream.trim().length > 0 ? upstream.trim() : '（无上游原文）';
    return "".concat(zh, "\n\u4E0A\u6E38\u539F\u6587\uFF1A").concat(src, "\n\u4E0B\u4E00\u6B65\uFF1A").concat(next);
}
var GUARD_RE = /blockScratchBuild|htmlGenerated|notAvailable|空白|守卫|cannot add|scratch build|cloud generation|htmlToPptx|cloudGenStatus/i;
function classifyControlError(input) {
    var _a;
    var err = input.error;
    var path = (_a = input.path) !== null && _a !== void 0 ? _a : '';
    ...
    if (input.kind === 'relay' && err === 'executor not registered' || err.includes('executor not registered')) {
        return {
            ...
            message: triple("\u6587\u6863\u5C1A\u672A\u5728\u63A7\u5236\u6A21\u5F0F\u6253\u5F00".concat(path ? "\uFF08".concat(path, "\uFF09") : '', "\u3002"), err, '在侧栏 explorer / chat 产物行 / git 面板点击该文件，等控制模式 iframe 加载后再重试。'),
        };
    }
