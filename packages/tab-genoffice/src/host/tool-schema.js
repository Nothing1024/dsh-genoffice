...
exports.CONTROL_TOOL_TABLE = void 0;
exports.isSaveEntry = isSaveEntry;
...
var DOCS_CONTROL_NOTE = '该工具操作 GenOffice 网页版中已打开的 docx 文档（控制模式）。所有块索引基于文档当前状态，修改后索引会变化，需重新读取上下文。';
var MARKDOWN_CONTROL_NOTE = '该工具操作 GenOffice 网页版中已打开的 markdown 文档（控制模式）。markdown 内容必须是纯 GFM。';
var SHEETS_CONTROL_NOTE = '该工具操作 GenOffice 网页版中已打开的 xlsx 工作簿（控制模式）。所有编辑先改 iframe 内工作表，只有 xlsx_save 或 tab「写入磁盘」才会写回原文件。';
var SLIDES_CONTROL_NOTE = '该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。';
var PDF_CONTROL_NOTE = '该工具操作 GenOffice 网页版中已打开的 pdf 文档（控制模式）。所有标注/编辑先改 iframe 内状态，只有 pdf_save 或 tab「写入磁盘」才会写回原文件。页码 1 起。';
/**
...
exports.CONTROL_TOOL_TABLE = [
    ...
    // ── markdown (app: markdown) ─────────────────────────────────────
    {
        ...
        skillName: 'get_document_context',
        app: 'markdown',
        description: MARKDOWN_CONTROL_NOTE +
    ...
    // ── xlsx (app: sheets) ──────────────────────────────────────────────
    {
        ...
        app: 'sheets',
        description: SHEETS_CONTROL_NOTE +
    ...
    // ── pptx (app: slides) ──────────────────────────────────────────────
    {
        ...
        app: 'slides',
        description: SLIDES_CONTROL_NOTE +
    ...
    },
    {
        ...
        app: 'slides',
        description: SLIDES_CONTROL_NOTE + '设置页面背景色。slideIndex=-1 应用到所有页。',
        parameters: {
            path: PATH_PARAM,
