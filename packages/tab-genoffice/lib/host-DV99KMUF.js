...
async function executeInsertSlideImage(entry, input, signal, extraHeaders = {}) {
	...
	const mime = SLIDE_IMAGE_MIME[extname(imagePath).toLowerCase()];
	if (mime === void 0) fail("仅支持 png / jpeg / webp / gif", path, "local");
	if (!imagePath.startsWith("/") || imagePath.split("/").includes("..")) fail("imagePath 必须是本机绝对路径，且不得包含 ..", path, "local");
	let bytes;
	try {
