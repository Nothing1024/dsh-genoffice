...
async function callRelay(entry, input, signal, extraHeaders = {}) {
	...
	if (execution.isError && /missing images|配图失败|failed to download/i.test(output)) return {
		ok: false,
		output,
		summary: "落页成功，但有配图失败"
	};
	if (execution.isError) fail(output || "executor error", path, "executor");
	return {
		ok: true,
		output,
...
async function executeInsertSlideImage(entry, input, signal, extraHeaders = {}) {
	...
	let ext;
	try {
		ext = assertSafeImagePath(imagePath);
	} catch (e) {
		fail(e instanceof Error ? e.message : String(e), path, "local");
	}
	const mime = IMAGE_MIME[ext];
	if (mime === void 0) fail("仅支持 png / jpeg / webp / gif", path, "local");
	let bytes;
	try {
	...
	if (bytes.length > 20971520) fail("图片超过 20MB", path, "local");
...
async function executeLandPages(input, signal, settle, extraHeaders = {}) {
	try {
		...
		if (/missing images|配图失败|failed to download/i.test(result.output)) return {
			ok: false,
