...
const HOST_REQUIRED = [TOOL_REGISTRY];
const HOST_OPTIONAL = [
	SYSTEM_PROMPT,
	SKILL_REGISTRY,
	WEB_SERVER
...
function assertSafeImagePath(absPath) {
	...
	if (IMAGE_MIME[ext] === void 0) throw new Error("仅支持 png / jpeg / webp / gif");
	return ext;
}
async function assertImageFile(absPath) {
...
async function readLocalImageDataUrl(absPath) {
	const ext = await assertImageFile(absPath);
	const mime = IMAGE_MIME[ext];
	if (mime === void 0) throw new Error("仅支持 png / jpeg / webp / gif");
	return `data:${mime};base64,${(await readFile(absPath)).toString("base64")}`;
}
...
*/
function createAssetChannelFrom(acquire) {
	...
	const cancel = acquire === void 0 ? () => {} : acquire((http) => {
...
const CAPABILITY = {
	...
	"docs:apply_commands": {
		status: "available",
		netEgress: false,
		evidence: "docs/tools.ts apply_commands alias → apply_ops"
	},
	"docs:read_comments": {
		status: "available",
		netEgress: false,
		evidence: "docs/tools.ts read_comments via App comments store"
	},
	"docs:reply_comment": {
		status: "available",
		netEgress: false,
		evidence: "docs/tools.ts reply_comment via App comments store"
	},
	"docs:resolve_comment": {
		status: "available",
		netEgress: false,
		evidence: "docs/tools.ts resolve_comment via App comments store"
	},
	"docs:set_header_footer": {
		status: "available",
		netEgress: false,
		evidence: "docs/tools.ts set_header_footer via App hf access"
	},
	"docs:read_revisions": {
		status: "available",
		netEgress: false,
		evidence: "docs/tools.ts read_revisions via editor track"
	},
	"docs:web_search": {
		status: "relay-fetch",
		netEgress: true,
	...
	"markdown:apply_ops": {
		status: "available",
		netEgress: false,
		evidence: "markdown/tools.ts apply_ops official batch; insert_content/replace_blocks aliased"
	},
	"markdown:save": {
		status: "available",
		netEgress: false,
	...
	"slides:insert_web_image": {
		status: "available",
		netEgress: false,
		evidence: "host pptx_insert_image reads a local imagePath and hands the iframe a data:image URL; insert_web_image decodes it without a network fetch"
	},
	"slides:create": {
		status: "available",
		netEgress: false,
		evidence: "host pptx_create posts /api/pptx/create; relay writes the blank 13.333×7.5in deck (one undecorated slide) and refuses to overwrite"
	},
	"slides:crop_image": {
		status: "available",
		netEgress: false,
	...
	"html:get_outline": {
		status: "available",
		netEgress: false,
		evidence: "html/src/renderer/ai/tools.ts AGENT_TOOLS get_outline + control.ts executeTool"
	},
	"html:read_source": {
		status: "available",
		netEgress: false,
		evidence: "html/src/renderer/ai/tools.ts AGENT_TOOLS read_source + control.ts executeTool"
	},
	"html:apply_ops": {
		status: "available",
		netEgress: false,
		evidence: "html/src/renderer/ai/tools.ts AGENT_TOOLS apply_ops + control.ts executeTool"
	},
	"html:save": {
		status: "available",
		netEgress: false,
		evidence: "relay POST /api/control/html/<docId>/export"
	},
	"html:export_docx": {
		status: "available",
		netEgress: false,
		evidence: "host html_export_docx → GET /api/html/docx/ready then job; dest only after PK zip"
	}
};
...
function classifyControlError(input) {
	...
	if (input.kind === "relay" && err === "executor not registered" || err.includes("executor not registered")) return {
		class: "executor-missing",
		message: triple(`文档尚未在控制模式打开${path ? `（${path}）` : ""}。`, err, "确认浏览器里的 DSH 页面开着，再调用一次对应 *_open。不要对同一次失败连续重试；连续重试不会注册执行器。")
	};
...
const CONTROL_TOOL_TABLE = [
	...
	},
	{
		...
		app: "docs",
		description: "该工具操作 GenOffice 网页版中已打开的 docx 文档（控制模式）。所有块索引基于文档当前状态，修改后索引会变化，需重新读取上下文。读取当前文档评论线程（id、作者、正文、锚点）。回复或解决前先调用本工具获取有效 id。",
		parameters: { path: PATH_PARAM }
	},
	{
		...
		app: "docs",
		description: "该工具操作 GenOffice 网页版中已打开的 docx 文档（控制模式）。所有块索引基于文档当前状态，修改后索引会变化，需重新读取上下文。向已有评论线程追加一条回复。parentId 必须是 read_comments 返回的线程 id。",
		parameters: {
			path: PATH_PARAM,
			parentId: {
				type: "string",
				required: true,
				description: "目标评论线程 id"
			},
			text: {
				type: "string",
				required: true,
	...
	},
	{
		...
		app: "docs",
		description: "该工具操作 GenOffice 网页版中已打开的 docx 文档（控制模式）。所有块索引基于文档当前状态，修改后索引会变化，需重新读取上下文。将评论线程标记为已解决。id 必须是 read_comments 返回的线程或回复 id。",
		parameters: {
			path: PATH_PARAM,
			id: {
				type: "string",
				required: true,
				description: "评论或回复 id"
			}
		}
	},
	{
		...
		app: "docs",
		description: "该工具操作 GenOffice 网页版中已打开的 docx 文档（控制模式）。所有块索引基于文档当前状态，修改后索引会变化，需重新读取上下文。设置页眉或页脚文本。kind=header|footer；view=default|first|even。",
		parameters: {
			path: PATH_PARAM,
			kind: {
				type: "string",
				required: true,
				description: "header 或 footer"
			},
			text: {
				type: "string",
				required: true,
				description: "页眉/页脚文本，最多 2000 字"
			},
			view: {
				type: "string",
				description: "default / first / even，默认 default"
			}
		}
	},
	{
		...
		app: "docs",
		description: "该工具操作 GenOffice 网页版中已打开的 docx 文档（控制模式）。所有块索引基于文档当前状态，修改后索引会变化，需重新读取上下文。读取当前文档修订/跟踪更改摘要，不修改文档。",
		parameters: { path: PATH_PARAM }
	},
	{
	...
	},
	{
		...
		skillName: "apply_ops",
		app: "markdown",
		description: "该工具操作 GenOffice 网页版中已打开的 markdown 文档（控制模式）。markdown 内容必须是纯 GFM。按官方 apply_ops 批量执行 insertContent/replaceBlocks 等操作。索引写入前如有用户编辑需先 get_document_context。",
		parameters: {
			path: PATH_PARAM,
			ops: {
				type: "array",
	...
	},
	{
		...
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。把本机图片插入指定页。imagePath 必须是本机绝对路径（png/jpeg/webp/gif）；宿主把它发到 loopback 再插入，不要传 http(s) URL，也不要自建静态服务。x/y/w/h 为画布像素（1280×720）。",
		parameters: {
			path: PATH_PARAM,
			slideIndex: {
				type: "integer",
			...
			x: {
				type: "number",
				required: true,
				description: "左边距，画布像素"
			},
			y: {
				type: "number",
				required: true,
				description: "上边距，画布像素"
			},
			w: {
				type: "number",
				required: true,
	...
	},
	{
		...
		skillName: "create",
		app: "slides",
		description: "新建一张空白 pptx（标准 13.333×7.5 英寸、1 页、无母版装饰）并写入 path。已存在的文件会拒绝覆盖。写完后必须再调用 pptx_open。不要复制现有 pptx 当容器：底版的版式装饰会进成稿。",
		parameters: { path: PATH_PARAM }
	},
	{
	...
	},
	{
		...
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。按 brief 由当前会话模型写一页 PageSpec，再 land_pages replace_at（其他页不动）。也可直接传 page_spec 跳过规划。不要把 brief 交给 iframe 打 LLM。先 read_slide；需要图时把真实 http(s) URL 放进 image_urls。超时或断线只回报结果不确定并请先读稿核实，不会自动重放写入。",
		parameters: {
			path: PATH_PARAM,
			slideIndex: {
				type: "integer",
	...
	},
	{
		...
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。把宿主写好的 PageSpec[] 落入当前稿（iframe 只落地，不打 LLM）。pages 每页须有 elements[]，元素含 type/x/y/w/h；画布 1280×720。insert_mode 缺省 replace；replace_at/insert_at 需 pages.length===1 且整数 at_index。落地不写盘。超时或断线只回报结果不确定并请先读稿核实，不会自动重放写入。",
		parameters: {
			path: PATH_PARAM,
			pages: {
				type: "array",
	...
	},
	{
		...
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。用当前 DSH 会话模型写 PageSpec[]，再 land_pages 落地（不转发 iframe generate_deck，不配 iframe key）。推荐 topic + approx_pages；也可直接传 pages_spec。空白稿出片后解锁 add_text_box / add_shape。落地不写盘。超时或断线只回报结果不确定并请先读稿核实，不会自动重放写入。",
		parameters: {
			path: PATH_PARAM,
			topic: {
				type: "string",
	...
	},
	{
		...
		skillName: "add_chart",
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。插入原生图表。kind=bar/barStacked/line/area/pie/doughnut。categories 为分类；series 为 {name, values}；dataSource=user/document/search/sample（必填，sample 须向用户声明为示意）。省略 x/y/w/h 时居中。",
		parameters: {
			path: PATH_PARAM,
			slideIndex: {
				type: "integer",
	...
	},
	{
		...
		skillName: "add_table",
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。插入原生表格。rows/cols 含表头；cells 为按行文本（可选，缺省为空）。省略 x/y/w/h 时居中。插入后用返回的 element id 调用 edit_table_cell/structure/style。",
		parameters: {
			path: PATH_PARAM,
			slideIndex: {
				type: "integer",
	...
	},
	{
		...
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。整格替换表格单元格文本。sourceId 为表格元素 id；row/col 从 0 起；paragraphs 为富文本段落数组。",
		parameters: {
			path: PATH_PARAM,
			slideIndex: {
				type: "integer",
	...
	},
	{
		...
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。增删表格行列。kind=insert-row/delete-row/insert-col/delete-col；index 为 0 起行列号；insert 默认插在其后，before=true 插在其前。",
		parameters: {
			path: PATH_PARAM,
			slideIndex: {
				type: "integer",
			...
			index: {
				type: "integer",
				required: true,
				description: "行/列号（0 起）"
			},
			before: {
				type: "boolean",
				description: "insert 时为 true 则插在 index 之前"
			}
		}
	},
	{
		...
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。修改表格样式。styleName 预设优先：none/lightGrid/zebraBlue/zebraGray/headerDarkBlue/headerOrange/noBorder/fullBorder。也可单独设表头行、隔行、底纹和边框。",
		parameters: {
			path: PATH_PARAM,
			slideIndex: {
				type: "integer",
			...
			shadingColor: {
				type: "string",
				description: "底纹 #RRGGBB，none 清除"
			},
			borderColor: {
				type: "string",
				description: "边框 #RRGGBB"
			},
			borderWidthPt: {
				type: "number",
				description: "边框宽度 pt"
	...
	},
	{
		...
		skillName: "edit_chart",
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。修改图表类型/数据/配色。kind=bar/barStacked/line/area/pie/doughnut。传 series 时必须带 dataSource。",
		parameters: {
			path: PATH_PARAM,
			slideIndex: {
				type: "integer",
	...
	},
	{
		...
		skillName: "apply_ops",
		app: "slides",
		description: "该工具操作 GenOffice 网页版中已打开的 pptx 演示文稿（控制模式）。所有编辑先改 iframe 内幻灯片，只有 pptx_save 或 tab「写入磁盘」才会写回原文件。页面索引 0 起，画布 1280×720。以一笔事务应用官方原子编辑 op（默认 isolation=atomic：任一失败全回滚，文档不半改）。dry_run=true 只校验计划、不改稿。最多 50 个 op；空数组或超过 50 直接拒绝。寻址：每个 op 带 target:{slide, el?}。slide 为 0 起页码或耐久 id s_<n>；el 为 read_slide/outline 的元素 id（e_* 耐久）。组合子元素：target.el=子 id 且加 group:\"<group id>\"。几何单位是文档空间 EMU：1px=9525 EMU（read_slide 报 px，先乘 9525）。字号为 pt。不要把像素坐标直接当 EMU。常用 op：setText {paragraphs:[{runs:[{text}]}]} · setFont {font:{…}} · setFill {fill:\"#RRGGBB\"|\"none\"} · setStroke {stroke:{color,widthEmu}|null} · setTransform {box:{x,y,cx,cy},rotDeg?} · addElement {kind,offset} · setTableCell {row,col,paragraphs} · moveSlide {to} · findReplace {find,replace}（走 apply_ops，不要调单独的查找对话框）。未知 op 名会返回完整词表；失败 op 的错误带一行 Usage 签名。先 dry_run 再正式提交。单页排版优先 execute_slide_script，单个编辑优先专用工具。网页版 applyTxn→runTxn；per_op 失败跳过。",
		parameters: {
			path: PATH_PARAM,
			ops: {
				type: "array",
	...
	},
	{
		...
		skillName: "get_outline",
		app: "html",
		description: "该工具操作 GenOffice 网页版中已打开的 HTML 文档（控制模式）。编辑先改 iframe 内源码，只有 html_save 或 tab「写入磁盘」才会写回原文件。转 Word 前先查 genoffice_services / html_export_docx 可用性，不要引导安装桌面版。读取 HTML 结构大纲：每行给出行号范围、sid、标签和文本预览。大文档可用 from_sid / depth 收窄。",
		parameters: {
			path: PATH_PARAM,
			depth: {
				type: "integer",
				description: "展开深度，默认 3"
			},
			from_sid: {
				type: "integer",
				description: "只看该元素及其子树"
			}
		}
	},
	{
		...
		app: "html",
		description: "该工具操作 GenOffice 网页版中已打开的 HTML 文档（控制模式）。编辑先改 iframe 内源码，只有 html_save 或 tab「写入磁盘」才会写回原文件。转 Word 前先查 genoffice_services / html_export_docx 可用性，不要引导安装桌面版。按行号或 sid 读取带行号的 HTML 源码片段。长结果分页，page 从 0 起。",
		parameters: {
			path: PATH_PARAM,
	...
	},
	{
		...
		skillName: "apply_ops",
		app: "html",
		description: "该工具操作 GenOffice 网页版中已打开的 HTML 文档（控制模式）。编辑先改 iframe 内源码，只有 html_save 或 tab「写入磁盘」才会写回原文件。转 Word 前先查 genoffice_services / html_export_docx 可用性，不要引导安装桌面版。按官方 apply_ops 批量改 HTML，整批校验后全成或全败。ops 含 str_replace / replace_element / set_inner_html / set_text / insert_html / remove / move / set_attr / set_style。",
		parameters: {
			path: PATH_PARAM,
			ops: {
				type: "array",
	...
	},
	{
		...
		skillName: "save",
		app: "html",
		description: "该工具操作 GenOffice 网页版中已打开的 HTML 文档（控制模式）。编辑先改 iframe 内源码，只有 html_save 或 tab「写入磁盘」才会写回原文件。转 Word 前先查 genoffice_services / html_export_docx 可用性，不要引导安装桌面版。将当前 HTML 显式写回原文件（原子写回）。编辑工具只改网页内状态，只有本工具或 tab「写入磁盘」会真正写盘。",
		parameters: SAVE_PARAMS
	},
	{
		...
		app: "html",
		description: "该工具操作 GenOffice 网页版中已打开的 HTML 文档（控制模式）。编辑先改 iframe 内源码，只有 html_save 或 tab「写入磁盘」才会写回原文件。转 Word 前先查 genoffice_services / html_export_docx 可用性，不要引导安装桌面版。把当前 HTML 转为 Word 并写 dest。先查 /api/html/docx/ready；不可用时只报告 reason，不要求安装桌面版。非法 HTML、外链或取消不会写出 dest。",
		parameters: {
			path: PATH_PARAM,
			dest: {
				type: "string",
				description: "目标 docx 绝对路径；省略则与源同目录同名 .docx"
			}
		}
	}
];
...
const GENOFFICE_PROTOCOL = "genoffice-control";
const FAMILY_TO_APP = {
	docs: "docs",
	docx: "docs",
	word: "docs",
...
function resolveControlFamily(token) {
...
function shouldRegister(entry, opts) {
	...
	if ((entry.name === "docx_insert_image" || entry.name === "pdf_insert_image" || entry.name === "pdf_replace_image") && opts.assetsAvailable === false) return false;
	return true;
}
function contractHeaders(opts, entry) {
	if (opts.family == null && opts.schemaRevision == null) return {};
	const headers = {
		"X-GenOffice-Protocol": GENOFFICE_PROTOCOL,
		"X-GenOffice-Schema-Revision": opts.schemaRevision || "2026.09.1"
	};
	...
	while (Date.now() < deadline) {
		...
		try {
			...
			if (resp.ok) {
				...
				if (data.readiness == null && (data.registered === true || data.registered === void 0)) return true;
			}
		} catch (e) {
			if (signal.aborted) return false;
...
async function callRelay(entry, input, signal, extraHeaders = {}) {
	...
	if (execution.isError && !landedDespiteMissingImages(output)) fail(output || "executor error", path, "executor");
	return {
		ok: true,
		output,
...
async function saveViaRelay(entry, input, signal, extraHeaders = {}) {
	const path = String(input.path ?? "");
	if (!path.startsWith("/")) fail("path 必须是目标文件的本机绝对路径", path, "local");
	if (isInSyncWindow(path)) fail("sync window", path, "sync");
...
async function executeInsertSlideImage(entry, input, signal, extraHeaders = {}) {
	const path = String(input.path ?? "");
...
async function executeCreateBlankPptx(input, signal) {
	const path = String(input.path ?? "");
	if (!path.startsWith("/") || !path.toLowerCase().endsWith(".pptx")) fail("path 必须是尚不存在的 .pptx 本机绝对路径", path, "local");
	let resp;
	try {
	...
	if (data.ok !== true) {
		...
		if (err === "conflict") fail("目标 pptx 已存在，拒绝覆盖。换一个尚不存在的 path。", path, "local");
		fail(err, path, "relay");
	}
	return {
		ok: true,
...
async function executeInsertImage(entry, input, signal, assets, extraHeaders = {}) {
	const path = String(input.path ?? "");
	const imagePath = String(input.imagePath ?? "");
...
async function waitLanded(path, expectedPages, signal, needle, settle, extraHeaders = {}) {
...
const LAND_UNCERTAIN = "落页写入回执不确定（超时或断线）。本次最多发送一次写请求，不要重试本写入。请先 pptx_get_deck_context 或 pptx_read_slide 核对实际页数后再决定是否补做。";
const LAND_IMAGE_WARNING = "落页成功，但有配图失败";
function landedDespiteMissingImages(output) {
	return /missing images|配图失败|failed to download/i.test(output);
}
async function executeLandPages(input, signal, settle, extraHeaders = {}) {
	try {
		const result = await callRelay(landPagesEntry(), input, signal, extraHeaders);
		const expected = parseLandedCount(result.output);
		if (expected !== void 0) await waitLanded(String(input.path ?? ""), expected, signal, firstTextNeedle(input.pages), settle, extraHeaders);
		if (landedDespiteMissingImages(result.output)) return {
			ok: true,
		...
		if (isTimeoutError(e) || isAbortError(e)) return {
			ok: false,
			output: LAND_UNCERTAIN,
			summary: "落页结果待核对"
		};
		throw e;
	}
}
async function executeGenerateDeck(input, signal, planLlm, settle, extraHeaders = {}) {
	const path = String(input.path ?? "");
	let pages;
	try {
...
async function executeRegenerateSlide(input, signal, planLlm, settle, extraHeaders = {}) {
	const path = String(input.path ?? "");
...
function createControlTools(opts = {}) {
	...
	return [
		...CONTROL_TOOL_TABLE.filter((entry) => shouldRegister(entry, {
			allTools,
			assetsAvailable,
			familyApp,
			discoveryTools
			...
			return defineTool({
				...
				async execute(args, exec) {
					const input = args;
					if (entry.name === "docx_insert_image" || entry.name === "pdf_insert_image" || entry.name === "pdf_replace_image") return await executeInsertImage(entry, input, exec.signal, opts.assets, contractHeaders(opts, entry));
					if (entry.name === "pptx_insert_image") return await executeInsertSlideImage(entry, input, exec.signal, contractHeaders(opts, entry));
					if (entry.name === "pptx_create") return await executeCreateBlankPptx(input, exec.signal);
					if (isSave) {
					...
					if (entry.skillName === "land_pages") return await executeLandPages(input, exec.signal, settle, contractHeaders(opts, entry));
					if (entry.name === "html_export_docx") return await executeHtmlExportDocx(input, exec.signal, contractHeaders(opts, entry));
		...
		...createOpenTools(familyApp ? familyToken : null),
		...createServiceTools()
	];
}
...
async function executeHtmlExportDocx(input, signal, extraHeaders = {}) {
	const path = String(input.path ?? "");
	if (!path.startsWith("/")) fail("path 必须是目标文件的本机绝对路径", path, "local");
	const destHint = input.dest;
	const dest = typeof destHint === "string" && destHint.startsWith("/") ? destHint : `${path.replace(/\.html?$/i, "")}.docx`;
	let ready;
	try {
		ready = await (await fetch(`${RELAY_BASE}/api/html/docx/ready`, { signal })).json();
	} catch (e) {
		fail(e instanceof Error ? e.message : String(e), path, "fetch");
	}
	if (ready.available !== true) fail(String(ready.reason ?? "html-to-docx-unavailable"), path, "relay");
	let html = "";
	try {
		const file = await (await fetch(`${RELAY_BASE}/api/file?path=${encodeURIComponent(path)}`, { signal })).json();
		if (file.ok !== true || typeof file.base64 !== "string") fail(String(file.error ?? "html source unreadable"), path, "relay");
	...
	let started;
	try {
		started = await (await fetch(`${RELAY_BASE}/api/html/docx/jobs`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				...extraHeaders
	...
	if (started.ok !== true || !started.jobId) fail(String(started.error ?? "html-docx-rejected"), path, "relay");
	let done;
	try {
		done = await (await fetch(`${RELAY_BASE}/api/html/docx/jobs/wait`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				...extraHeaders
	...
	if (done.status === "cancelled" || done.canceled === true) fail("cancelled", path, "relay");
	if (done.ok !== true) fail(String(done.error ?? "html-docx-failed"), path, "relay");
	return {
		ok: true,
		output: `已转换为 ${done.dest ?? dest}`,
		summary: "HTML 已转为 Word"
	};
}
function createServiceTools() {
	return [defineTool({
		name: "genoffice_services",
		description: "规划前查询 GenOffice 网页服务是否可用（打印、OCR、HTML→Word、provider、文档加解密/附件提取、Markdown 资源）。不可用时只报告 reason，不要要求安装桌面版。",
		parameters: {},
		output: {
			schema: {
				type: "object",
		...
		async execute(_args, exec) {
			...
			try {
				...
				fail(e instanceof Error ? e.message : String(e), void 0, "fetch");
			}
			const data = await resp.json();
			const slice = {
				live: data.live,
...
function createOpenTools(family) {
	...
	return (familyApp ? [{
		...
		pdf: "pdf",
		html: "html"
	}[familyApp]] : OPEN_TOOL_EXTS).map((ext) => defineTool({
		name: `${ext}_open`,
		...
		async execute(args, exec) {
			...
			if (!(ext === "html" ? base.toLowerCase().endsWith(".html") || base.toLowerCase().endsWith(".htm") : base.toLowerCase().endsWith(`.${ext}`))) fail(`path 必须是 .${ext} 文件`, filePath, "local");
			let resp;
			try {
...
*   资产通道不可用而不注册）。
*/
var host_default = defineFacet((activation) => {
	const { contracts, scope, extensions } = activation;
	const web = contracts.has(WEB_SERVER) ? contracts.get(WEB_SERVER) : void 0;
	...
	if (contracts.has(SYSTEM_PROMPT)) scope.add(contracts.get(SYSTEM_PROMPT).section({
		...PROMPT_SECTION,
		text: buildGenOfficePromptText()
	}));
	if (contracts.has(SKILL_REGISTRY)) scope.add(contracts.get(SKILL_REGISTRY).register({
		name: GENOFFICE_SKILL_NAME,
		description: GENOFFICE_SKILL_DESCRIPTION,
		content: GENOFFICE_SKILL_CONTENT,
		source: "runtime"
	}));
	for (const tool of createControlTools({ assets })) extensions.publish(TOOL_REGISTRY, tool.name, tool);
});
//#endregion
export { HOST_OPTIONAL as a, SYSTEM_PROMPT as c, lookupSkills as d, lookupSystemPrompt as f, runFacet as i, TOOL_REGISTRY as l, coordKey as n, HOST_REQUIRED as o, lookupWebServer as p, createActivation as r, SKILL_REGISTRY as s, host_default as t, WEB_SERVER as u };
