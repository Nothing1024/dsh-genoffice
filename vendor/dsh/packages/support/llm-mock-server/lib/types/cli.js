...
import { MAX_MOCK_LLM_TIMER_DELAY_MS, MOCK_LLM_BEHAVIORS } from "./index.js";
...
const BEHAVIORS = new Set(MOCK_LLM_BEHAVIORS);
...
function numberValue(option, value) {
    ...
    if (!Number.isFinite(parsed))
        throw new Error(`dsh-llm-mock-server: ${option} must be a finite number`);
    return parsed;
}
...
function parseSequence(raw) {
    ...
    for (const entry of requestEntries) {
        if (!BEHAVIORS.has(entry))
            throw new Error(`dsh-llm-mock-server: unknown behavior ${JSON.stringify(entry)}`);
    }
    return { startsUnavailable, sequence: requestEntries };
}
