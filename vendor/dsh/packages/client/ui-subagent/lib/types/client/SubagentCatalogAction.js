...
export function SubagentCatalogAction({ sessionId, useSessions, openChild, refresh, setCatalogOpen, t, }) {
    ...
    return (_jsxs("div", { className: css.root, ref: rootRef, onKeyDown: navigate, children: [_jsxs("button", { ref: triggerRef, type: "button", className: css.trigger, "aria-haspopup": "tree", "aria-expanded": open, "aria-label": t(descendants.running ? runningCountKey : totalCountKey, { count: descendantCount }), onClick: () => { changeOpen(!open); }, onKeyDown: (event) => {
