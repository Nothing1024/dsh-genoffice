...
export function createFixtureApi(options = {}) {
    ...
    const modelTargets = new Map(sessions.map(session => [
        session.sessionId,
    ...
    const fxMutateGoal = (request, ref, next) => {
        ...
        return ok(request, { ref: { id: projection.goal.id, revision: projection.goal.revision } });
    };
    ...
    return {
        ...
        commands: {
            ...
            execute: (request) => {
                ...
                if (name === 'permission') {
                    ...
                    return ok(request, { matched: true, commandId });
                }
                if (name === 'goal') {
                ...
                if (name === undefined || text === undefined)
                    return ok(request, { matched: false });
                const commandId = `fx-cmd-${logOf(id).length}`;
                append(id, { type: 'command/run', data: { commandId, name, args, source: { kind: 'user' } } });
        ...
        goals: {
            ...
            resume: request => fxMutateGoal(request, request.payload.ref, current => (current.goal.phase === 'paused' || current.goal.phase === 'blocked' || current.goal.phase === 'active'
                ...
                : undefined)),
            complete: request => fxMutateGoal(request, request.payload.ref, current => (current.goal.phase === 'complete'
                ? undefined
