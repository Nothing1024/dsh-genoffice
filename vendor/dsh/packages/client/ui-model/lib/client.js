window.__ModuleLoader__.load({
	id: "@deepseek-ai/dsh-client-ui-model",
	factory: (require) => {
		var module = { exports: {} };
		...
		*/
		...
		var ModelService = class extends cordis.Service {
			...
			*/
			directoryFor(sessionId) {
				...
				if (actx === void 0) throw new Error(`ui-model: session "${String(sessionId)}" resolved no scope`);
		...
		//#endregion
		//#region \0dsh-css:/Users/nothing/workspace/dsh/wt-artifact/packages/client/ui-model/src/client/ModelSelect.module.css.mjs
		const css = ".mWp0SG_root{min-width:0;position:relative}.mWp0SG_trigger{min-width:0;max-width:220px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:24px;outline:none;align-items:center;gap:4px;padding:0 4px 0 8px;font-size:13px;font-weight:500;line-height:20px;display:flex}.mWp0SG_trigger:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}.mWp0SG_trigger:focus-visible{box-shadow:0 0 0 2px var(--dsw-alias-border-l3)}.mWp0SG_trigger:disabled{color:var(--dsw-alias-label-dimmed);cursor:default}.mWp0SG_triggerLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}.mWp0SG_triggerEffort{color:var(--dsw-alias-label-caption);flex:none}.mWp0SG_chevron{color:var(--dsw-alias-label-caption);flex:none;transition:transform .12s}.mWp0SG_chevronOpen{transform:rotate(180deg)}.mWp0SG_menu{z-index:20;border:1px solid var(--dsw-alias-border-inverted);background:var(--dsw-specific-menu);width:min(240px,100vw - 32px);max-height:min(360px,100vh - 96px);box-shadow:var(--dsw-shadow-lv3);color:var(--dsw-alias-label-primary);--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);border-radius:12px;flex-direction:column;padding:4px;display:flex;position:absolute;bottom:calc(100% + 8px);right:0;overflow:hidden}.mWp0SG_status,.mWp0SG_empty{color:var(--dsw-alias-label-tertiary);padding:10px;font-size:13px;line-height:20px}.mWp0SG_error,.mWp0SG_warning{background:var(--dsw-alias-interactive-bg-hover-danger);color:var(--dsw-alias-state-error-primary);border-radius:8px;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:4px;padding:7px 8px;font-size:12px;line-height:18px;display:flex}.mWp0SG_warning{background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-state-warn-label)}.mWp0SG_retry{color:inherit;font:inherit;cursor:pointer;background:0 0;border:none;flex:none;padding:0;font-weight:600}.mWp0SG_groups{min-height:0;overflow-y:auto}.mWp0SG_group+.mWp0SG_group{margin-top:4px}.mWp0SG_groupTitle{z-index:1;background:var(--dsw-specific-menu);color:var(--dsw-alias-label-tertiary);padding:5px 8px 3px;font-size:12px;font-weight:500;line-height:18px;position:sticky;top:0}.mWp0SG_option{width:100%;min-height:38px;color:inherit;text-align:left;cursor:pointer;background:0 0;border:none;border-radius:10px;outline:none;align-items:center;gap:8px;padding:6px 8px;display:flex}.mWp0SG_option:hover:not(:disabled),.mWp0SG_option:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.mWp0SG_selected{background:0 0}.mWp0SG_option:disabled{color:var(--dsw-alias-label-dimmed);cursor:default}.mWp0SG_optionCopy{flex-direction:column;flex:1;min-width:0;display:flex}.mWp0SG_modelName{color:inherit;text-overflow:ellipsis;white-space:nowrap;font-size:14px;font-weight:500;line-height:20px;overflow:hidden}.mWp0SG_description{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:12px;line-height:18px;overflow:hidden}.mWp0SG_check{color:var(--dsw-alias-label-primary);flex:0 0 18px;place-items:center;display:grid}.mWp0SG_cell{width:100%;height:40px;color:var(--dsw-alias-label-primary);cursor:pointer;text-align:left;background:0 0;border:none;border-radius:10px;align-items:center;gap:8px;padding:0 10px;font-size:14px;line-height:22px;display:flex}.mWp0SG_cell:hover{background:var(--dsw-alias-interactive-bg-hover)}.mWp0SG_cellLabel{text-overflow:ellipsis;white-space:nowrap;flex:auto;min-width:0;overflow:hidden}.mWp0SG_cellValue{text-overflow:ellipsis;white-space:nowrap;min-width:0;color:var(--dsw-alias-label-tertiary);flex:0 auto;overflow:hidden}.mWp0SG_cellChevron{color:var(--dsw-alias-label-tertiary);flex:none}";
		const tagId = "@deepseek-ai/dsh-client-ui-model/ModelSelect.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@deepseek-ai/dsh-client-ui-model";
			tag.dataset.pluginCss = tagId;
			tag.textContent = css;
			document.head.appendChild(tag);
		}
		var ModelSelect_module_css_default = {
			"trigger": "mWp0SG_trigger",
			"status": "mWp0SG_status",
			"optionCopy": "mWp0SG_optionCopy",
			"description": "mWp0SG_description",
			"triggerLabel": "mWp0SG_triggerLabel",
			"menu": "mWp0SG_menu",
			"triggerEffort": "mWp0SG_triggerEffort",
			"warning": "mWp0SG_warning",
			"groups": "mWp0SG_groups",
			"group": "mWp0SG_group",
			"check": "mWp0SG_check",
			"cellValue": "mWp0SG_cellValue",
			"error": "mWp0SG_error",
			"chevronOpen": "mWp0SG_chevronOpen",
			"retry": "mWp0SG_retry",
			"option": "mWp0SG_option",
			"empty": "mWp0SG_empty",
			"chevron": "mWp0SG_chevron",
			"root": "mWp0SG_root",
			"groupTitle": "mWp0SG_groupTitle",
			"cellChevron": "mWp0SG_cellChevron",
			"modelName": "mWp0SG_modelName",
			"selected": "mWp0SG_selected",
			"cellLabel": "mWp0SG_cellLabel",
			"cell": "mWp0SG_cell"
		};
		//#endregion
		...
		*/
		function ModelSelect({ locked, available, directory, load, select, t }) {
			...
			const currentChoice = choices[state.current === null ? -1 : choices.findIndex((c) => c.target.provider === state.current?.provider && c.target.model === state.current.model)];
		...
		*/
		function apply(ctx) {
			...
			ctx.effect(() => ctx.locale.register(NS, {
				zh,
				en
			}), "ui-model: dictionaries");
			const t = ctx.locale.bind(NS);
			ctx.inject(["command", "models"], (scope) => {
				const command = scope.get("command");
				...
				scope.effect(() => command.register({
					...
					ui: {
						...
						onSelect: async (option, session) => {
							...
							if (target === void 0) throw new Error("this provider's catalog failed to load — pick a model from a loaded group");
							await directory.select(target);
						}
					}
				}), "ui-model: /model contribution");
			});
			ctx.inject(["slots", "models"], (scope) => {
				const models = scope.models;
				const sessions = scope.sessions;
				scope.slots.inject("conversation.input.model", () => scope.slots.register({
					...
					inject: (sessionId) => {
						...
						return {
							...
							select: (target) => available ? directory.select(target).then(() => true, () => false) : Promise.resolve(false)
						};
					}
				}, ModelSelect));
			});
