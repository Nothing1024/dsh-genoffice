...
import { LOCAL_FILE_OPEN_EVENT, splitTextWithLocalPaths } from './local-paths.ts'
 ...
 * @param context - The pass state.
 * @param plainText - Render text nodes as raw strings (no local-file linking);
 * used for link labels so a path can never nest an interactive element inside
 * the anchor.
 * @returns One React node per rendered child.
 */
function renderChildren(
  ...
  return nodes.map((node, index) => renderNode(node, index, context, plainText))
}

function renderNode(node: Md.RootContent, key: Key, context: MarkdownRenderContext, plainText = false): ReactNode {
  switch (node.type) {
    case 'text':
      // Local-file linking: absolute md/mdx/docx paths render as clickable
      // local-file links (dispatch LOCAL_FILE_OPEN_EVENT); everything else
      // stays plain text (local-paths.ts). Link labels opt out (plainText)
      // so a path inside an authored link can never nest an interactive
      // element inside the anchor.
      return plainText ? node.value : renderLocalPathText(node.value, key)
    case 'paragraph':
    ...
    case 'link':
      return renderAnchor(node.url, renderChildren(node.children, context, true), key)
    case 'linkReference':
...
/**
 * Render one text-node value, splitting out detected local file paths into
 * clickable local-file links (local-paths.ts). A click dispatches
 * {@link LOCAL_FILE_OPEN_EVENT} on `window` with the path as `detail`; host
 * plugins subscribe to preview the file. Paths are inert without a listener —
 * the link is a styled button that never navigates.
 * @param value - The raw text-node value.
 * @param key - The render key for the fragment.
 * @returns The text run with path segments rendered as links.
 */
function renderLocalPathText(value: string, key: Key): ReactNode {
  const tokens = splitTextWithLocalPaths(value)
      ...
      {tokens.map((token, index) => token.kind === 'text'
        ...
        : (
          <button
            key={index}
            type="button"
            className={css.localFileLink}
            title={token.path}
            onClick={(event) => {
              event.preventDefault()
              event.stopPropagation()
              window.dispatchEvent(new CustomEvent<string>(LOCAL_FILE_OPEN_EVENT, { detail: token.path }))
            }}
          >
            {token.path}
          </button>
...
function renderLinkReference(
  ...
  const children = renderChildren(node.children, context, true)
