...
describe('local-file path linking', () => {
  it('renders a detected path as a clickable button that dispatches the open event', () => {
    const received: string[] = []
    const listener = (event: Event): void => {
      received.push((event as CustomEvent<string>).detail)
    }
    window.addEventListener('dsh:open-local-file', listener)
    try {
      const container = renderNodes([
        { type: 'paragraph', children: [text('见 /Users/nothing/报告.md 和普通文本')] },
      ])
      expect(container.textContent).toBe('见 /Users/nothing/报告.md 和普通文本')
      const link = container.querySelector('button')
      expect(link?.getAttribute('title')).toBe('/Users/nothing/报告.md')
      link?.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }))
      expect(received).toEqual(['/Users/nothing/报告.md'])
    } finally {
      window.removeEventListener('dsh:open-local-file', listener)
    }
  })

  it('keeps non-path text and link labels as plain text', () => {
    const container = renderNodes([
      { type: 'paragraph', children: [
        text('普通文本 https://example.com/x.md 和 docs/a.md'),
      ] },
    ])
    expect(container.textContent).toBe('普通文本 https://example.com/x.md 和 docs/a.md')
    expect(container.querySelector('button')).toBeNull()
    const anchored = renderNodes([
      {
        type: 'paragraph',
        children: [
          { type: 'link', url: 'https://example.com', children: [text('标签 /x/a.md 不嵌套')] },
        ],
      },
    ])
    expect(anchored.querySelector('button')).toBeNull()
    expect(anchored.querySelector('a')?.textContent).toBe('标签 /x/a.md 不嵌套')
  })
})
