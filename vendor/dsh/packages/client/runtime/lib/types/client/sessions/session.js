...
import { isVisibleAssistantChunk, PartialAccumulator } from "./partial.js";
...
export class Session {
    ...
    acceptLiveEvent(event, view) {
        ...
        if (event.type === 'assistant/chunk') {
            if (isVisibleAssistantChunk(event.data.chunk.type))
    ...
    applyEventSideEffects(event, view) {
        ...
        if (eventType === 'artifact/delta') {
            // Host-side dsh-tool-artifact declares this log-only event; this wire
            // consumer narrows it structurally (same posture as code-dispatch).
        ...
        switch (event.type) {
            ...
            case 'tool/result': {
                if (this.openCalls.delete(String(event.data.message.source.callId)))
                    this.callsRev++;
                return;
            }
            case 'turn/end': {
    ...
    settleScheduledRetry(retryState, turn) {
        const index = this.derivedNodes.findLastIndex(node => node.kind === 'model-retry'
    ...
    buildSnapshot() {
        ...
        if (this.deltasCache === null || this.deltasCache.rev !== this.deltasRev) {
            this.deltasCache = { rev: this.deltasRev, value: new Map(this.artifactDeltas) };
        }
        ...
        return {
            ...
            nodes.some(node => node.kind !== 'command') || partial !== null || this.running || this.pendingCache.value.length > 0, this.promptAttempted),
...
function parseRetryEventData(value) {
    ...
    if (!nonNegativeSafeInteger(data.turn)
        ...
        || failureData.code.length === 0)
        return null;
    if (data.mode === 'normal') {
        if (!positiveSafeInteger(data.maxRetries) || data.retry > data.maxRetries)
            return null;
    }
