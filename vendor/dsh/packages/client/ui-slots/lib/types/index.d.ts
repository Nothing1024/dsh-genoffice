 ...
 */
export interface SlotSpec<E extends SlotEntryDef> {
    kind: E['kind'];
    scope: E['scope'];
}
/**
 ...
 */
export type PropsRuntime<K extends keyof SlotMap & string> = OwnerOf<K> & (ScopeOf<K> extends 'session' ? SessionStandardProps : ScopeOf<K> extends 'session-maybe' ? SessionMaybeStandardProps : object) & GlobalStandardProps;
 ...
 */
export type ComposedProps<K extends keyof SlotMap & string, S extends keyof SlotMap & string, H, I extends object, M = never, N = undefined> = PropsRuntime<K> & PropsRenderSlots<S> & PropsStore<H> & InjectFace<I> & MatchedShare<SlotMap[K], M> & PropsLocale<N>;
/**
...
export type KindOptions<E extends SlotEntryDef, M = never> = E['kind'] extends 'keyed' ? {
    key: string;
} : E['kind'] extends 'list' ? {
    id: string;
    order?: number;
...
export declare class SlotCore {
     ...
     */
    register<K extends keyof SlotMap & string, const D extends ChildrenDecl = Record<never, never>, H extends StoreDecl | undefined = undefined, M = never, N extends (keyof LocaleNamespaceMap & string) | undefined = undefined, C extends SlotComponent<never> = SlotComponent<never>>(options: BaseOptions<K, D, H, M, N> & {
        inject?: undefined;
    }, component: C & SlotComponent<ComposedProps<K, keyof NoInfer<D> & keyof SlotMap & string, HandleOf<NoInfer<H>>, object, NoInfer<M>, NoInfer<N>>> & RendersCheck<C, D>): () => void;
    /**
     ...
     * @returns disposer removing the registration and its declarations.
     */
    register<K extends keyof SlotMap & string, I extends object, const D extends ChildrenDecl = Record<never, never>, H extends StoreDecl | undefined = undefined, M = never, N extends (keyof LocaleNamespaceMap & string) | undefined = undefined, C extends SlotComponent<never> = SlotComponent<never>>(options: BaseOptions<K, D, H, M, N> & {
        inject: (...args: InjectParams<K, H>) => I;
    }, component: C & SlotComponent<ComposedProps<K, keyof NoInfer<D> & keyof SlotMap & string, HandleOf<NoInfer<H>>, I, NoInfer<M>, NoInfer<N>>> & RendersCheck<C, D>): () => void;
    /**
