...
export function ProviderEditor(props) {
    ...
    const curatedFields = (family) => {
        ...
        return (_jsxs(_Fragment, { children: [_jsxs("div", { className: styles['field'], children: [_jsx("span", { className: styles['fieldLabel'], children: t('keyInput') }), _jsx("input", { className: styles['input'], type: "password", autoComplete: "off", value: keyDraft, placeholder: keyPlaceholder, "aria-label": t('keyInput'), disabled: disabled || keyLocked, onChange: (event) => { setKeyDraft(event.target.value); } }), keyFailure === undefined ? null : _jsx("p", { className: styles['error'], children: t(keyFailure) })] }), _jsxs("details", { className: styles['customized'], children: [_jsx("summary", { className: styles['customizedSummary'], children: t('customized') }), _jsxs("div", { className: styles['customizedBody'], children: [ownsIdentity
    ...
    return (_jsxs("div", { className: styles['editor'], children: [props.hideTitle === true
                ...
                : curatedFields(layout), failure !== undefined ? _jsx("p", { className: styles['error'], children: failure }) : null, modelFailure === undefined
                ? null
                : (_jsx("p", { className: styles['advancedHint'], children: `${t('model')} ${String(modelFailure.index + 1)}: ${t(modelFailure.key)}` })), _jsx(EditorFooter, { t: t, busy: busy, submitDisabled: disabled || layout === 'unknown' || modelFailure !== undefined
                    || keyFailure !== undefined, submitLabel: "apply", submitBusyLabel: "applying", onCancel: () => { props.onClose(false); }, onSubmit: () => { void apply(); } })] }));
}
