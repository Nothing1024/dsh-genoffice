...
function SessionEntry({ entry, ownerProps, info }) {
    ...
    return _jsx(Comp, { ...kit, ...injected, ...ownerProps });
}
...
function SessionMaybeEntry({ entry, ownerProps }) {
    ...
    return _jsx(SessionMaybeEntryBody, { entry: entry, ownerProps: ownerProps, info: info }, epoch);
}
...
function StrictSessionEntry({ slotKey, entry, ownerProps }) {
    ...
    if (info.sessionId === undefined)
        return null;
    return (_jsx(SlotErrorBoundary, { slotKey: slotKey, children: _jsx(SessionEntry, { entry: entry, ownerProps: ownerProps, info: info }) }, info.sessionId));
}
function SlotOutlet({ slotKey, ownerProps, opts }) {
    ...
    const guarded = (entry, key, owner = ownerProps) => (spec.scope === 'session'
        ? _jsx(StrictSessionEntry, { slotKey: slotKey, entry: entry, ownerProps: owner }, key)
        : (_jsx(SlotErrorBoundary, { slotKey: slotKey, children: spec.scope === 'session-maybe'
                ? _jsx(SessionMaybeEntry, { entry: entry, ownerProps: owner })
                : _jsx(RootEntry, { entry: entry, ownerProps: owner }) }, key)));
    if (spec.kind === 'single') {
    ...
    if (list.length === 0)
        return _jsx(_Fragment, { children: opts?.fallback ?? null });
    return _jsx(_Fragment, { children: list.map((item, i) => guarded(item.entry, item.id ?? i)) });
}
...
function RootOutlet({ ownerProps }) {
    ...
    return (_jsx(SlotErrorBoundary, { slotKey: "root", children: _jsx(RootEntry, { entry: entry, ownerProps: ownerProps }) }));
}
/**
