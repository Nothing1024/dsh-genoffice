window.__ModuleLoader__.load({
	id: "@deepseek-ai/dsh-client-ui-command",
	factory: (require) => {
		var module = { exports: {} };
		...
		*/
		...
		var CommandService = class extends cordis.Service {
			static inject = [
				"slash",
				"sessions",
				"connection"
			...
			*/
			constructor(ctx) {
				...
				if (connection === void 0) throw new Error("ui-command: connection service unavailable");
				...
				if (slash === void 0) throw new Error("ui-command: slash service unavailable");
			...
			*/
			register(contribution) {
				const dispose = this.ctx.effect(() => {
					...
					if (contributions.has(contribution.name)) throw new Error(`ui-command: duplicate contribution for /${contribution.name}`);
			...
			*/
			decorate(decoration) {
				const dispose = this.ctx.effect(() => {
					...
					if (decorations.has(decoration.name)) throw new Error(`ui-command: duplicate decoration for /${decoration.name}`);
			...
			*/
			...
			async candidates(session, req) {
				...
				for (const contribution of this.live.contributions.values()) {
					...
					if (seen.has(contribution.name)) throw new Error(`ui-command: contribution /${contribution.name} collides with a host command`);
					rows.push({
			...
			*/
			async execute(session, line) {
				const { result } = await this.ctx.get("connection").api.commands.execute({
			...
			*/
			...
			sessions() {
				...
				if (sessions === void 0) throw new Error("ui-command: sessions service unavailable");
				return sessions;
			}
		};
		//#endregion
		...
		//#endregion
		//#region \0dsh-css:/Users/nothing/workspace/dsh/wt-artifact/packages/client/ui-command/src/client/PopupSelectView.module.css.mjs
		const css = ".T50h0a_card{z-index:100;--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);border:1px solid var(--dsw-alias-border-inverted);background:var(--dsw-specific-menu);min-width:min(220px,100%);max-width:100%;max-height:320px;box-shadow:var(--dsw-shadow-lv3);border-radius:12px;outline:none;flex-direction:column;padding:4px;display:flex;position:absolute;bottom:calc(100% + 4px);left:0;overflow:hidden}.T50h0a_viewport{flex-direction:column;min-height:0;display:flex;overflow-y:auto}.T50h0a_row{cursor:pointer;color:var(--dsw-alias-label-primary);border-radius:8px;align-items:center;gap:8px;padding:6px 8px;font-size:13px;display:flex}.T50h0a_rowActive{background:var(--dsw-alias-interactive-bg-hover)}.T50h0a_label{white-space:nowrap;text-overflow:ellipsis;flex:auto;min-width:0;overflow:hidden}.T50h0a_detail{color:var(--dsw-alias-label-tertiary);white-space:nowrap;text-overflow:ellipsis;font-size:12px;overflow:hidden}.T50h0a_check{color:var(--dsw-alias-label-primary);flex:none;display:inline-flex}.T50h0a_status{color:var(--dsw-alias-label-tertiary);padding:8px 10px;font-size:13px}.T50h0a_search{border:1px solid var(--dsw-alias-border-inverted);color:var(--dsw-alias-label-primary);background:0 0;border-radius:8px;outline:none;margin:2px 2px 4px;padding:6px 8px;font-size:13px}.T50h0a_error{color:var(--dsw-alias-state-error-primary);align-items:center;gap:8px;padding:6px 8px;font-size:12px;display:flex}.T50h0a_errorText{text-overflow:ellipsis;flex:1;overflow:hidden}.T50h0a_retry{border:1px solid var(--dsw-alias-border-inverted);color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border-radius:6px;padding:2px 8px;font-size:12px}";
		const tagId = "@deepseek-ai/dsh-client-ui-command/PopupSelectView.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@deepseek-ai/dsh-client-ui-command";
			tag.dataset.pluginCss = tagId;
			tag.textContent = css;
			document.head.appendChild(tag);
		}
		var PopupSelectView_module_css_default = {
			"detail": "T50h0a_detail",
			"viewport": "T50h0a_viewport",
			"error": "T50h0a_error",
			"errorText": "T50h0a_errorText",
			"rowActive": "T50h0a_rowActive",
			"check": "T50h0a_check",
			"status": "T50h0a_status",
			"search": "T50h0a_search",
			"retry": "T50h0a_retry",
			"label": "T50h0a_label",
			"card": "T50h0a_card",
			"row": "T50h0a_row"
			...
			ctx.inject([
				...
				scope.slots.inject("conversation.input.overlay", () => scope.slots.register({
					...
					inject: (sessionId) => {
						...
						if (actx === void 0) throw new Error(`ui-command: session "${String(sessionId)}" resolved no scope`);
