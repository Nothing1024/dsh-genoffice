window.__ModuleLoader__.load({
	id: "@deepseek-ai/dsh-client-ui-slash",
	factory: (require) => {
		var module = { exports: {} };
		...
		const menuReduce = (state, ev) => {
			switch (ev.type) {
				...
				case "source-settled": {
					...
					const groups = state.groups.map((g, i) => i !== idx ? g : {
						source: g.source,
						status: "ready",
						items
					});
		...
		*/
		...
		var SlashService = class extends cordis.Service {
			...
			*/
			...
			sessions() {
				...
				if (sessions === void 0) throw new Error("ui-slash: sessions service unavailable");
				return sessions;
			}
		};
		//#endregion
		...
		//#endregion
		//#region \0dsh-css:/Users/nothing/workspace/dsh/wt-artifact/packages/client/ui-slash/src/client/MenuView.module.css.mjs
		const css = ".jtYTBq_menu{z-index:100;--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);border:1px solid var(--dsw-alias-border-inverted);background:var(--dsw-specific-menu);min-width:min(260px,100%);max-width:min(537px,100%);max-height:320px;box-shadow:var(--dsw-shadow-lv3);border-radius:12px;flex-direction:column;padding:4px;display:flex;position:absolute;bottom:calc(100% + 4px);left:0;overflow:hidden}.jtYTBq_viewport{flex-direction:column;min-height:0;display:flex;overflow-y:auto}.jtYTBq_item{cursor:pointer;width:100%;min-height:40px;color:var(--dsw-alias-label-primary);text-align:left;background:0 0;border:none;border-radius:10px;align-items:center;gap:8px;padding:8px 10px;font-size:14px;line-height:22px;display:flex}.jtYTBq_item:hover,.jtYTBq_item.jtYTBq_active{background:var(--dsw-alias-interactive-bg-hover)}.jtYTBq_itemIcon{width:16px;height:16px;color:var(--dsw-alias-label-tertiary);flex:none;justify-content:center;align-items:center;display:inline-flex}.jtYTBq_itemName{text-overflow:ellipsis;white-space:nowrap;flex:none;max-width:40%;overflow:hidden}.jtYTBq_itemDescription{text-overflow:ellipsis;white-space:nowrap;min-width:0;color:var(--dsw-alias-label-tertiary);flex:1;overflow:hidden}.jtYTBq_groupTitle{color:var(--dsw-alias-label-tertiary);padding:8px 10px;font-size:12px;line-height:16px}.jtYTBq_loading{min-height:40px;color:var(--dsw-alias-label-dimmed);align-items:center;padding:8px 10px;font-size:14px;line-height:22px;display:flex}";
		const tagId = "@deepseek-ai/dsh-client-ui-slash/MenuView.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@deepseek-ai/dsh-client-ui-slash";
			tag.dataset.pluginCss = tagId;
			tag.textContent = css;
			document.head.appendChild(tag);
		}
		var MenuView_module_css_default = {
			"viewport": "jtYTBq_viewport",
			"menu": "jtYTBq_menu",
			"itemDescription": "jtYTBq_itemDescription",
			"groupTitle": "jtYTBq_groupTitle",
			"loading": "jtYTBq_loading",
			"itemIcon": "jtYTBq_itemIcon",
			"active": "jtYTBq_active",
			"item": "jtYTBq_item",
			"itemName": "jtYTBq_itemName"
		};
		//#endregion
		...
		*/
		function MenuView({ menu, onPick, onDismiss, t }) {
			const state = (0, react.useSyncExternalStore)((fn) => menu.subscribe(fn), () => {
		...
		*/
			...
			ctx.inject([
				...
				scope.slots.inject("conversation.input.overlay", () => scope.slots.register({
					...
					inject: (sessionId) => {
						...
						if (actx === void 0) throw new Error(`ui-slash: session "${String(sessionId)}" resolved no scope`);
		...
		//#endregion
		exports.SlashController = SlashController;
		exports.SlashService = SlashService;
		exports.apply = apply;
