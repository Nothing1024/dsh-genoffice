...
function parseDshClient(pkgName, value) {
	...
	if (typeof value !== "object" || value === null) throw new Error(`client-modules: ${pkgName} has a non-object dshClient declaration`);
	const decl = value;
	if (typeof decl.platform !== "string") throw new Error(`client-modules: ${pkgName} dshClient.platform must be a string`);
	if (decl.inject !== void 0 && (!Array.isArray(decl.inject) || decl.inject.some((i) => typeof i !== "string"))) throw new Error(`client-modules: ${pkgName} dshClient.inject must be a string array`);
	if (decl.immediately !== void 0 && typeof decl.immediately !== "boolean") throw new Error(`client-modules: ${pkgName} dshClient.immediately must be a boolean`);
	return {
...
var ClientModuleHostService = class extends Service {
	...
	*/
	...
	resolveMeta(pkgName) {
		...
		if (clientRel === void 0) throw new Error(`client-modules: ${pkgName} declares dshClient but exports no "./client" bundle`);
		const meta = {
