...
export class ClientModuleHostService extends Service {
    ...
    constructor(ctx) {
        ...
        ctx.effect(() => ctx.httpServer.register({ kind: 'prefix', path: '/plugins', handler: this.serveBundle }), 'client-modules: bundle route');
        ctx.effect(() => { return ctx.httpServer.tapIndex(html => injectBootManifest(html, this.composed)); }, 'client-modules: boot manifest injection');
    }
    /**
