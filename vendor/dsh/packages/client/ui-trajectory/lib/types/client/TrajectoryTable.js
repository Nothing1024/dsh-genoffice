...
import { trajectoryPreviewText } from "./layout.js";
...
function MessageOrigin({ record }) {
    ...
    if (source === undefined)
        return _jsx("p", { className: css.noPayload, children: "Origin not recorded" });
    const data = typeof source === 'object' && source !== null
        ? source
        : { value: source };
    return (_jsx(JsonTree, { data: data, label: "Message origin JSON", className: css.jsonPayload }));
}
...
export function TrajectoryTable({ requestNumbers: sessionRequestNumbers, turns, streamingCells = [], timelineFocusIndexes = null, searchMatchIndexes = null, onSelectedIndexChange, onRecordSelect, recordSelection = null, recordFocus = null, historyLoading = false, historyStartSeq, hasOlderRecords = false, onLoadOlder, onClearSelection, collapsedTurns, onToggleTurn, collapsedAssistants, onToggleAssistant, inspectCallId = null, onInspectApplied, }) {
    ...
    const requestNumbers = useMemo(() => indexRequestNumbers(allRecords, sessionRequestNumbers), [allRecords, sessionRequestNumbers]);
    ...
    const requestOlder = useCallback((pane) => {
        if (!hasOlderRecords
    ...
    return (_jsxs("div", { ref: rootRef, className: css.split, style: splitStyle, children: [_jsxs("div", { ref: tablePaneRef, className: css.tablePane, "data-trajectory-scroll": "", onScroll: (event) => {
                    ...
                    if (event.target === event.currentTarget)
                        clearAllSelections();
                }, children: [showLoading && (_jsx("div", { className: css.historyLoading, role: "status", "aria-live": "polite", children: _jsxs("span", { className: css.historyLoadingBar, children: [_jsx("span", { className: css.historyLoadingSpinner, "aria-hidden": "true" }), loadingLabel] }) })), _jsxs("table", { className: css.table, "data-scroll-ready": tableScrollReady || undefined, "aria-rowcount": records.length, children: [_jsxs("colgroup", { children: [_jsx("col", { className: css.eventColumn }), _jsx("col", { className: css.contentColumn })] }), _jsxs("tbody", { children: [virtualTop > 0 && (_jsx("tr", { className: css.virtualSpacer, "data-virtual-spacer": "top", "aria-hidden": "true", children: _jsx("td", { colSpan: 2, style: {
                                        ...
                                        return (_jsxs("tr", { tabIndex: isRequestOnly ? -1 : 0, "aria-rowindex": position + 1, "aria-label": isCollapsedSummary
                                                ...
                                                selectRecord(record.cell.index);
                                                        ...
                                                        : record.collapsedSummary !== undefined
                                                            ...
                                                            : (_jsxs("span", { className: record.cell.result === undefined ? css.contentText : css.resultPreview, title: record.cell.result === undefined
                                                                    ? listDisplayText
                                                                    : `${listDisplayText} → ${record.cell.result}`, children: [_jsx("span", { className: record.cell.result === undefined ? undefined : css.resultRequest, children: toolCallOnly
                                                                            ? _jsx("span", { className: css.toolCallOnly, children: "(tool call only)" })
                                                                            : toolCallText === undefined
                                                                                ? listDisplayText || '—'
                                                                                : (_jsxs(_Fragment, { children: [_jsx("span", { className: css.toolCallNameTypeface, children: toolCallText.name || '—' }), toolCallText.args !== undefined && (_jsx("span", { className: css.toolCallPayload, children: toolCallText.args }))] })) }), record.cell.result !== undefined && (_jsxs("span", { className: record.cell.isError ? `${css.inlineResult} ${css.error}` : css.inlineResult, children: [_jsx("span", { className: css.arrow, children: "\u2192" }), _jsx("span", { className: record.cell.result === 'No output'
                                                                                    ? `${css.inlineResultText} ${css.noOutputText}`
                                                                                    : css.inlineResultText, children: record.cell.result })] }))] })) })] }, trajectoryVirtualRecordKey(record)));
                                    }), virtualBottom > 0 && (_jsx("tr", { className: css.virtualSpacer, "data-virtual-spacer": "bottom", "aria-hidden": "true", children: _jsx("td", { colSpan: 2, style: {
                ...
                || (selected !== undefined && selectedState !== undefined)) && (_jsxs("aside", { className: css.details, "aria-label": "Event details", style: detailsWidth === null ? undefined : { width: detailsWidth }, children: [_jsx("div", { className: css.detailsResizeHandle, role: "separator", "aria-label": "Resize event details", "aria-controls": "trajectory-detail-panel", "aria-orientation": "vertical", tabIndex: 0, title: "Drag to resize. Double-click to reset.", onDoubleClick: () => {
                            ...
                            : css.detailBody, role: "tabpanel", "aria-labelledby": `trajectory-detail-${activeTab}`, children: [selectedRequest !== null
                                ...
                                && selected.cell.kind !== 'compacted'
                                && selectedState !== undefined
                                && activeTab === 'overview' && (_jsxs(_Fragment, { children: [_jsxs("dl", { className: `${css.overview} ${css.summaryScrollRegion}`, "data-summary-scroll-region": "", children: [selected.cell.messageSource !== undefined && (_jsxs("div", { children: [_jsx("dt", { children: "Origin" }), _jsx("dd", { className: css.overviewParentLinks, children: _jsxs("button", { type: "button", className: css.overviewHierarchyNavLink, onClick: () => { activateTab('origin'); }, children: [_jsx("span", { children: messageOriginLabel(selected.cell.messageSource) }), _jsx(IconChevronRightOutline14, { className: css.overviewHierarchyJumpIconTight, size: 11 })] }) })] })), hasSelectedHierarchy && (_jsxs("div", { children: [_jsx("dt", { children: selectedAssistantRequestTarget !== undefined
                                                            ? 'Origin'
