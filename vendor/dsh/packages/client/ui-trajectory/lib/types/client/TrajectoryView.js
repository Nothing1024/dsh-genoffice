...
import { deriveTrajectoryContextBranches, trajectoryBranchContainsRequest, } from "./context-branches.js";
...
function mergeSearchMatches(finalized, partial) {
    if (finalized === null || partial === null)
        return null;
    return new Set([...finalized, ...partial]);
}
export function TrajectoryView({ useHistory, useDuration, loadHistoryTail, loadOlderHistory, setActualDuration, inspect, onInspectDone, }) {
    ...
    const contexts = useMemo(() => historyContexts.length === 0
        ? [{ id: 0, nodes }]
        : historyContexts, [historyContexts, nodes]);
    const branches = useMemo(() => { return deriveTrajectoryContextBranches(contexts); }, [contexts]);
    const currentBranch = branches.at(-1);
    if (currentBranch === undefined)
        throw new Error('trajectory branch projection must not be empty');
    ...
    const selectedRequests = useMemo(() => requests.filter(request => trajectoryBranchContainsRequest(currentBranch, request)), [currentBranch, requests]);
    ...
    const finalizedSearchMatches = useMemo(() => searchMatches(finalized.turns, searchQuery), [finalized, searchQuery]);
    ...
    const partialSearchMatches = useMemo(() => searchMatches(partialSearchTurns, searchQuery), [partialSearchTurns, searchQuery]);
    const searchMatchIndexes = useMemo(() => mergeSearchMatches(finalizedSearchMatches, partialSearchMatches), [finalizedSearchMatches, partialSearchMatches]);
