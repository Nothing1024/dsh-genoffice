...
export function ConversationSessionHeader({ sessionId, useSession, useSessions, useStore, actions, renderSlot, views, open, t, }) {
    ...
    return (_jsx("header", { className: clsx(css.header, hideChrome && css.headerHidden), "aria-hidden": hideChrome || undefined, children: !hideChrome && (_jsxs(_Fragment, { children: [_jsxs("div", { className: css.titleRow, children: [_jsxs("nav", { className: css.crumbs, "aria-label": t('session.hierarchy'), children: [ancestry.map((summary, index) => {
 ...
 */
export function ConversationSession({ useSession, useInput, inputActions, useStore, actions, renderSlot, views, bindDraftMirror, }) {
    useSyncExternalStore(views.subscribe, views.version);
    const tabs = views.list();
    const activeId = useStore(s => s.view) ?? 'chat';
    const active = tabs.find(view => view.id === activeId) ?? tabs[0];
