...
export function TodoPanel({ todos, t }) {
    ...
    if (todos.length === 0)
        return null;
    return (_jsx("section", { className: css.root, "data-testid": "todo-panel", "aria-label": t('todo.title'), children: _jsxs("div", { className: css.body, children: [_jsxs("button", { type: "button", className: css.header, "aria-expanded": !collapsed, onClick: () => { setCollapsed(v => !v); }, children: [_jsx("span", { className: css.lead, "aria-hidden": true, children: _jsx(IconChecklistOutline14, {}) }), _jsx("span", { className: css.title, children: t('todo.title') }), _jsx("span", { className: css.progress, children: progressLabel(todos, t) }), _jsx("span", { className: css.chevron, "aria-hidden": true, children: !collapsed ? _jsx(IconChevronDownOutline14, {}) : _jsx(IconChevronUpOutline14, {}) })] }), !collapsed && (_jsx("ul", { className: css.list, children: todos.map(item => (_jsxs("li", { className: css.item, "data-status": item.status, children: [_jsx("span", { className: css.glyph, "aria-hidden": true, children: _jsx(StatusGlyph, { status: item.status }) }), _jsx("span", { className: css.content, children: item.content })] }, item.content))) }))] }) }));
}
