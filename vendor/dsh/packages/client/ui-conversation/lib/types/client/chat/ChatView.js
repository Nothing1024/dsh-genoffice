...
import { assistantActionsSeqs, deriveChatFlow, messageBranchSeqs, runningTurnStartTime } from "./chat-flow.js";
import { AssistantMarkdown } from "./AssistantMarkdown.js";
import { GenericCommandCard } from "./GenericCommandCard.js";
import { GenericToolCard } from "./GenericToolCard.js";
import { MessageItem, PendingSteeringBubble } from "./MessageItem.js";
import { formatRunDuration } from "./message-chrome.js";
import { deriveTurnMetrics } from "./turn-metrics.js";
...
function activeRetrySeq(nodes, running) {
    ...
    for (let index = nodes.length - 1; index >= 0; index -= 1) {
        ...
        if (node.kind === 'model-retry')
            return node.retryState === 'cancelled' ? null : node.seq;
...
const SubCallRow = memo(function SubCallRow({ renderSlot, node, openFile, openDetails, closeDetails, selected, cwd, inspectCall, t }) {
    ...
    const owner = useMemo(() => ({
        ...
        selected,
        openDetails: () => { openDetails({ callId: node.callId }); },
        closeDetails,
        inspect: () => { inspectCall(node.callId); },
    }), [node, toolName, openFile, cwd, selected, openDetails, closeDetails, inspectCall]);
    return (_jsx("div", { className: css.callRow, "data-chat-anchor-key": `call:${node.callId}`, "data-chat-call-id": node.callId, "data-selected": selected || undefined, children: renderSlot('conversation.chat.toolview', owner, {
            entryKey: toolName,
...
const CallRow = memo(function CallRow({ renderSlot, callId, toolName, block, openFile, openDetails, closeDetails, selected, subCalls, selectedCallId, cwd, inspectCall, t, }) {
    const owner = useMemo(() => ({
        callId, toolName, block, openFile, cwd,
        selected,
        openDetails: () => { openDetails({ callId }); },
        closeDetails,
        inspect: () => { inspectCall(callId); },
    }), [callId, toolName, block, openFile, cwd, selected, openDetails, closeDetails, inspectCall]);
    return (_jsxs("div", { className: css.callRow, "data-chat-anchor-key": `call:${callId}`, "data-chat-call-id": callId, "data-selected": selected || undefined, children: [renderSlot('conversation.chat.toolview', owner, {
                entryKey: toolName,
                fallback: _jsx(GenericToolCard, { ...owner, t: t }),
            }), subCalls !== undefined && subCalls.length > 0 && (_jsx("div", { className: css.subCalls, "data-subcalls": true, children: subCalls.map(node => (_jsx(SubCallRow, { renderSlot: renderSlot, node: node, openFile: openFile, openDetails: openDetails, closeDetails: closeDetails, selected: node.callId === selectedCallId, cwd: cwd, inspectCall: inspectCall, t: t }, node.callId))) }))] }));
});
/** Consecutive tool results as one step-run group (uniform 16px rhythm). */
const ToolGroup = memo(function ToolGroup({ renderSlot, results, openFile, openDetails, closeDetails, selectedCallId, codeDispatches, cwd, inspectCall, t, }) {
    return (_jsx("div", { className: css.toolGroup, children: results.map(node => (_jsx(CallRow, { renderSlot: renderSlot, callId: node.callId, toolName: node.call?.name ?? '', block: node, openFile: openFile, openDetails: openDetails, closeDetails: closeDetails, selected: node.callId === selectedCallId, subCalls: codeDispatches.get(node.callId), selectedCallId: selectedCallId, cwd: cwd, inspectCall: inspectCall, t: t }, node.callId))) }));
});
...
function StreamingTail({ useSession, t }) {
    ...
    if (partial === null)
        return null;
    return _jsx(AssistantMarkdown, { blocks: partial.blocks, streaming: true, t: t });
}
/**
 * The chat view slot entry: pure component over the composed props (tool rows
 * render through the declared keyed hole's renderSlot share).
 */
export function ChatView({ useSession, useSessions, useStore, renderSlot, sessionId, openFile, openDetails, closeDetails, loadOlder, inspectCall, chatScroll, forkAt, t, }) {
    ...
    const wheelStartRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const onWheel = (event) => {
            ...
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    if (wheelEpochRef.current === epoch)
                        wheelStartRef.current = null;
                });
            });
        };
    ...
    const renderItem = (item) => {
        if (item.kind === 'tool-group') {
            ...
            return (_jsx(ToolGroup, { renderSlot: renderSlot, results: item.results, openFile: openFile, openDetails: openDetails, closeDetails: closeDetails, selectedCallId: inGroup ? selectedCallId : undefined, codeDispatches: codeDispatches, cwd: cwd, inspectCall: inspectCall, t: t }));
        }
        ...
        if (node.kind === 'assistant') {
            ...
            return (_jsx(AssistantMarkdown, { blocks: node.blocks, streaming: false, interrupted: node.interrupted, time: actionSeqs.has(node.seq) ? node.time : undefined, runMs: timing?.endTime === undefined
                    ? undefined
                    : Math.max(0, timing.endTime - timing.startTime), ttftMs: metrics?.ttftMs, tokensPerSecond: metrics?.tokensPerSecond, seq: node.seq, onFork: forkAt, forkUnavailable: !branchSeqs.has(node.seq), t: t }));
        }
        if (node.kind === 'command') {
            return _jsx(CommandRow, { renderSlot: renderSlot, node: node, t: t });
        }
        /* v8 ignore next -- tool-result never reaches here: deriveChatFlow folds them into groups. */
        if (node.kind === 'tool-result')
            return null;
        return (_jsx(MessageItem, { node: node, retryActive: node.kind === 'model-retry' && node.seq === activeRetry, onFork: forkAt, forkUnavailable: !branchSeqs.has(node.seq), t: t }));
    };
    return (_jsx("div", { className: css.root, children: _jsxs("div", { ref: listRef, className: css.scroll, children: [_jsxs("div", { ref: columnRef, className: css.column, "data-chat-flow": "", children: [openState === 'loading' && _jsx("div", { className: css.hint, children: t('chat.loadingHistory') }), openState === 'error' && openError !== null && (_jsx("div", { className: css.openError, children: t('chat.loadError', { message: openError.message, code: openError.code }) })), hasMore && (_jsx("div", { className: css.older, children: _jsx("button", { type: "button", disabled: loadingOlder, onClick: loadOlderAnchored, children: loadingOlder ? t('loading') : t('chat.loadOlder') }) })), items.map(item => (_jsx("div", { className: css.flowItem, "data-chat-anchor-key": item.kind === 'node' ? `node:${String(item.node.seq)}` : undefined, "data-chat-flow-key": item.key, "data-chat-flow-kind": item.kind === 'node' ? item.node.kind : 'tool-group', children: renderItem(item) }, item.key))), _jsx(StreamingTail, { useSession: useSession, t: t }), runningCalls.length > 0 && (_jsx("div", { className: css.toolGroup, children: runningCalls.map(call => (_jsx(CallRow, { renderSlot: renderSlot, callId: call.callId, toolName: call.name, block: call, openFile: openFile, openDetails: openDetails, closeDetails: closeDetails, selected: call.callId === selectedCallId, subCalls: codeDispatches.get(call.callId), selectedCallId: selectedCallId, cwd: cwd, inspectCall: inspectCall, t: t }, call.callId))) })), running && _jsx(TurnStatus, { startTime: runningTurnStart, t: t }), pendingSteering.map(item => (_jsx(PendingSteeringBubble, { content: item.content, t: t }, item.id)))] }), !atBottom && (_jsx("div", { className: css.toBottomSlot, children: _jsx("button", { type: "button", className: css.toBottom, "aria-label": t('chat.toBottom'), onClick: () => {
                            const local = listRef.current;
