 ...
 */
export function MessageIconActions({ text, time, runMs, ttftMs, tokensPerSecond, clock, onBranch, branchUnavailable = false, showBranch = true, className, t, }) {
    ...
    return (_jsxs("div", { className: className === undefined ? css.actions : `${css.actions} ${className}`, children: [clock === 'start' ? clockEl : null, _jsx(Tooltip, { label: copied ? t('copied') : t('copy'), side: "bottom", children: _jsx("button", { type: "button", className: css.action, "aria-label": copied ? t('copied') : t('copy'), onClick: onCopy, children: copied ? _jsx(IconCheckOutline16, {}) : _jsx(IconCopyOutline16, {}) }) }), showBranch && onBranch !== undefined && (_jsx(Tooltip, { label: branchUnavailable ? t('message.branchUnavailable') : t('message.branch'), side: "bottom", children: _jsx("button", { type: "button", className: css.action, "aria-label": t('message.branch'), "aria-disabled": branchUnavailable || undefined, "aria-describedby": branchUnavailable ? reasonId : undefined, "data-unavailable": branchUnavailable || undefined, onClick: branchUnavailable ? undefined : onBranch, children: _jsx(IconBranchOutline16, {}) }) })), showBranch && onBranch !== undefined && branchUnavailable && (_jsx("span", { id: reasonId, className: css.visuallyHidden, children: t('message.branchUnavailable') })), clock === 'end' ? clockEl : null] }));
}
