...
import type { InjectFace, MaybeSnapshotSelectorHook, PropsLocale, PropsRenderSlots, PropsRuntime, PropsStore, SnapshotSelectorHook } from '@deepseek-ai/dsh-client-ui-slots';
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
    interface SlotMap {
        ...
        /**
         * The details panel's per-tool output hole: keyed dispatch on the wire
         * tool name. Declared by the details entry (declaring is claiming); the
         * render site dispatches via `entryKey: toolName` with the panel's
         * card-model rendering as the `fallback` for unregistered tools.
         */
        'conversation.details.toolview': {
            kind: 'keyed';
            scope: 'session';
...
export interface ToolRowOwnerProps {
    ...
    /**
     * Select the call and open the details panel in one gesture (the chat
     * view's own `openDetails` injection: store `select` + layout
     * `openDetails`). Optional: registrants that render their own selection
     * affordance (e.g. an artifact card) call it; rows that never surface the
     * gesture leave it undefined and nothing changes.
     */
    openDetails?: ((target: SelectionTarget) => void) | undefined;
    /** Whether this call is the chat store's current selection (BR-003 toggle). */
    selected?: boolean | undefined;
    /**
     * Clear the selection and close the details panel (the chat view's
     * `closeDetails` injection: store `select(null)` + layout `closeDetails`).
     * Optional, paired with `selected` for the click-again-to-close gesture.
     */
    closeDetails?: (() => void) | undefined;
    /**
...
/**
 * Owner share of the details panel's per-tool output hole: the selected
 * call's slice — the settled result node, or the running call while it is
 * in flight (registrants discriminate with `'kind' in block`). Settled
 * registrants read the artifact contract off `block.meta` and the full
 * content off `block.content`; running registrants can stream progress from
 * the session snapshot's `artifactDeltas`.
 */
export interface DetailsToolOwnerProps {
    /** Tool call identity of the selected call (details linkage). */
    callId: CallId;
    /** Wire tool name (also the keyed dispatch key at the render site). */
    toolName: string;
    /** Frozen call slice: the running call or the settled result node. */
...
/** Full props of a registered details-toolview component. */
export type DetailsToolProps = PropsRuntime<'conversation.details.toolview'>;
/**
 ...
 */
export type ConversationSlotProps = PropsRuntime<'conversation'> & PropsRenderSlots<'conversation.session' | 'conversation.session.header' | 'conversation.composer' | 'conversation.composer.bar' | 'conversation.input.overlay' | 'conversation.input.dock' | 'conversation.composer.dock' | 'conversation.input.left' | 'conversation.input.right' | 'conversation.hero.workspace'> & ConversationInjected & PropsLocale<'conversation'>;
...
export interface ChatViewInjected {
    ...
    /** Selection clear + details panel closing in one gesture (BR-003 toggle partner). */
    closeDetails: () => void;
    /**
     * Open a tool-arg filesystem path with the host OS default application
...
export type ChatViewSlotProps = PropsRuntime<'conversation.view'> & PropsRenderSlots<'conversation.chat.toolview' | 'conversation.chat.commandview'> & PropsStore<ChatStore> & ChatViewInjected & PropsLocale<'conversation'>;
/**
...
export type DetailsSlotProps = PropsRuntime<'details'> & PropsRenderSlots<'conversation.details.toolview'> & PropsStore<ChatStore> & DetailsInjected & PropsLocale<'conversation'>;
