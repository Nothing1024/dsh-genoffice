...
/**
 * Selection target for the details linkage channel (toolcall is the step
 * special case). Only `callId` is consumed today (the details panel locates
 * call material by it); `turnSeq`/`stepSeq` are the reserved trajectory
 * coordinates and are optional because a tool row's settled node carries no
 * turn number.
 */
