 ...
 */
export function rehydrateSchema(serialized) {
 ...
 */
export function validateDraft(schema, draft) {
    try {
        ;
        schema(draft);
        return undefined;
...
export function setPath(root, path, value) {
    if (path.length === 0)
        throw new Error('schema-form: setPath needs a non-empty path');
...
export function deletePath(root, path) {
    if (path.length === 0)
        throw new Error('schema-form: deletePath needs a non-empty path');
