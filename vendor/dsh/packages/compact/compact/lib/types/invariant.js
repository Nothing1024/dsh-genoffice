...
function validateCompactionEvent(trace, event, fail) {
    ...
    if (event.type === 'compact/summary') {
        if (open === undefined)
            fail('compact/summary has no matching compact/start');
        validateOwner(open.turn, trace.openTurn, event.type, fail);
        if (open.summarized)
            fail('compact/summary repeated within one compaction');
        const seqs = event.data.shadowedSeqs;
        if (seqs.length === 0)
            fail('compact/summary shadowedSeqs must be non-empty');
    ...
    if (open === undefined)
        fail('compact/end has no matching compact/start');
...
const install = Object.assign((ctx, fail) => {
    ...
    const traceFor = (session) => { return traces.get(session) ?? seed(session); };
