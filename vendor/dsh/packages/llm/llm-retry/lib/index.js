...
function apply(ctx, config = {}, internals = {}) {
	...
	async function backoff(agent, turn, step, failure, provider, policy, policyKey, retry, delayMs, signal) {
	...
	async function recover({ agent, turn, step, provider, failure, retryPolicy: policy, signal }, next) {
		...
		const previousRetry = agent.session.events.findLast((event) => event.type === "llm/retry" && event.data.turn === turn && event.data.step === step && event.data.provider === provider && event.data.policyKey === policyKey)?.data.retry ?? 0;
