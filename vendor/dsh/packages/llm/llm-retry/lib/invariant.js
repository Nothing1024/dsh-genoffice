...
function validateRetry(history, event, fail) {
	...
	const expectedRetry = (history.findLast((prior) => prior.type === "llm/retry" && prior.data.turn === turn && prior.data.step === step && prior.data.provider === provider && prior.data.policyKey === policyKey)?.data.retry ?? 0) + 1;
