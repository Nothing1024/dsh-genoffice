...
var TokenMeterService = class extends Service {
	...
	_foldEvent(session, state, event) {
		...
		switch (event.type) {
			...
			case "step/start":
				...
				break;
			case "step/end":
				if (state.stepStart === void 0 || state.stepStart.turn !== event.data.turn || state.stepStart.step !== event.data.step) throw new Error(`token meter: step/end at seq ${event.seq} has no matching step/start boundary`);
		...
		if (event.type === "assistant/message") {
			...
			if (stepStart === void 0 || stepStart.turn !== event.data.turn || stepStart.step !== event.data.step) throw new Error(`token meter: assistant/message at seq ${event.seq} has no matching step/start boundary`);
...
//#endregion
export { TokenMeterService, TokenMeterService as default };
