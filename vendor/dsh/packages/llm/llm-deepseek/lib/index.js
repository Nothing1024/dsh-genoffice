...
import { CONTEXT_WINDOW_EXCEEDED_CODE, CallId, EMPTY_RESPONSE_CODE, LlmAdapter, LlmError, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId, RetryPolicySchema, attributionHeaders, isContextWindowExceededError, isQuotaExceededError, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
function apply(ctx, config) {
	...
	const resolveApiKey = async (connection) => {
		if (connection.apiKey !== void 0) return connection.apiKey;
		...
		throw new LlmError(`llm-deepseek: no API key for provider route "${PROVIDER}"; store ${ref} through the credentials service (the web Models page writes it), export ${ref} in the launching environment, or — as a last resort — set a literal "apiKey" in the llm-deepseek settings section`, "MISSING_CREDENTIAL");
	};
