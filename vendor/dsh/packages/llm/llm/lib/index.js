...
function callConfigEquals(a, b) {
	...
	return a.stop.length === b.stop.length && a.stop.every((s, i) => {
		return s === b.stop?.[i];
	});
}
/**
...
function errorChain(value) {
	...
	const render = (current) => {
		...
		try {
			...
			const message = current.message !== "" ? current.message : current.name;
...
*/
var LlmService = class extends Service {
	...
	*/
	listProviders() {
		return [...this.adapters.values()].map(({ provider }) => {
			return { ...provider };
		});
	}
	/**
...
//#endregion
export { APP_IDENTITY, BlockAssembler, CONTEXT_SUMMARY_MAX_CHARS, CONTEXT_WINDOW_EXCEEDED_CODE, CallId, EMPTY_RESPONSE_CODE, HarnessError, LlmAdapter, LlmError, LlmService, LlmService as default, MessageId, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId, RetryPolicySchema, assertNever, attributionHeaders, boundContextSummary, callConfigEquals, createAssistantMessage, createMessage, createToolResultMessage, createUserMessage, deepFreeze, errorChain, freezeMessage, isAgentLoopRequest, isContextWindowExceededError, isHarnessError, isQuotaExceededError, markAgentLoopRequest, resolveRetryPolicy, userAgent };
