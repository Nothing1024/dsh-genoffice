...
export class DomainFacility {
    ...
    async open(spec) {
        ...
        try {
            ...
            if (!backend.kv) {
                throw new DomainError('facet-unsupported', `backend '${backendName}' routed for domain '${spec.name}' has no kv facet`);
            }
...
function parseRecord(domain, table, key, parse) {
    try {
        ...
        throw new DomainError('invalid-record', `domain '${domain}': stored ${slot} does not match its schema`, { detail: { table, key }, cause: error });
    }
}
/**
