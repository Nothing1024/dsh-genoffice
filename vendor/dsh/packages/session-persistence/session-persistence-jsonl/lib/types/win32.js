 ...
 */
export async function publishNewFileWin32(existing, replacement) {
 ...
 */
export async function ensureDurableDirectoryWin32(target) {
    ...
    for (const segment of segments) {
        ...
        if (!await assertDirectory(next))
            await createLeafDirectoryWin32(current, next);
        current = next;
    }
}
async function createLeafDirectoryWin32(parent, target) {
    ...
    try {
        ...
        if (isEEXIST(error) && await assertDirectory(target))
            return;
        throw error;
    }
}
