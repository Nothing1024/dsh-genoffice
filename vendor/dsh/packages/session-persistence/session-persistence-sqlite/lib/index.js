...
import { DEFAULT_PREPARED_SESSION_CACHE_SIZE, PersistenceCoordinator, SessionPersistence, SessionPersistenceRevision } from "@deepseek-ai/dsh-session-persistence";
...
function configureDatabase(db, path, journalMode) {
	...
	try {
		...
		if (onDisk !== 0 && onDisk !== 13) throw new Error(`session database at "${path}" has schema version ${onDisk}, incompatible with this build (13)`);
		if (onDisk === 13 && applicationId !== 1146308688) throw new Error(`session database at "${path}" has application id ${applicationId}, expected ${SESSION_PERSISTENCE_SQLITE_APPLICATION_ID}`);
		db.exec(`
...
function surfaceBindings(event) {
	const se = event;
	return [se.sourceEventSeqs ? JSON.stringify(se.sourceEventSeqs) : null, se.surfaceOp !== void 0 ? JSON.stringify(se.surfaceOp) : null];
}
...
*/
var SessionPersistenceSqlite = class extends SessionPersistence {
	config;
	...
	*/
	...
	constructor(ctx, config) {
		...
		this.coordinator = new PersistenceCoordinator(this.ctx, this, { preparedSessionCacheSize });
	}
	...
	*/
	async loadStoredFrom(id, fromSeq, signal) {
		...
		const eventRows = this.db.prepare("SELECT seq, type, time, data, source_event_seqs, surface_op FROM events WHERE session_id = ? AND seq >= ? ORDER BY seq").all(id, fromSeq);
		signal?.throwIfAborted();
	...
	async readPrefix(id, signal) {
		...
		try {
			...
			if (row !== void 0) snapshot = {
				row,
				eventRows: this.db.prepare("SELECT seq, type, time, data, source_event_seqs, surface_op FROM events WHERE session_id = ? ORDER BY seq").all(id)
			};
			this.db.exec("COMMIT");
		} catch (error) {
	...
	*/
	async appendBatch(meta, events, isMaterialized) {
		await this.ready;
		const insertEvent = this.db.prepare("INSERT INTO events (session_id, seq, type, time, data, source_event_seqs, surface_op) VALUES (?, ?, ?, ?, ?, ?, ?)");
		this.db.exec("BEGIN");
		try {
	...
	*/
    ...
    `).run(meta.id, meta.version, meta.createdAt, meta.cwd ?? null, meta.parentSession ?? null, meta.seedLength ?? null, meta.origin ?? null, meta.delegationDepth ?? null, randomUUID());
	}
};
//#endregion
export { SCHEMA_VERSION, SessionPersistenceSqlite, SessionPersistenceSqlite as default };
