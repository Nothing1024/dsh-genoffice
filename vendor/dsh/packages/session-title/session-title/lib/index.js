	...
	*/
	...
	validateResult(result, messages) {
		...
		if (modelCandidate !== void 0) {
			if (modelCandidate === null || typeof modelCandidate !== "object") throw new Error("session-title provider model provenance requires non-empty provider and model");
			const record = modelCandidate;
			if (typeof record.provider !== "string" || record.provider.length === 0 || typeof record.model !== "string" || record.model.length === 0) throw new Error("session-title provider model provenance requires non-empty provider and model");
			model = {
