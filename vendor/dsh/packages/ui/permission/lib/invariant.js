...
const name = "permission-invariant";
/** Service required before the companion can reserve package ownership. */
const inject = ["invariants"];
/** Validate the package-owned event shape and ignore unrelated events. */
function validateEvent(ctx, event, fail) {
	if (event.type === "permission/preset" && !ctx.permission.names.includes(event.data.preset)) fail(`permission/preset names unknown preset ${JSON.stringify(event.data.preset)}`);
}
