...
*/
var PermissionService = class extends Service {
	...
	constructor(ctx, config) {
		...
		if (ctx.bash.sandboxMode === void 0) throw new Error("permission: the mounted bash executor does not confine (no sandboxMode) — presets bundle a sandbox mode, so composing this plugin over an unconfined executor is a misconfiguration");
	...
	*/
	...
	apply(session, name, setApproval) {
		...
		if (spec.sandbox !== (effectiveSandboxMode(events) ?? this.ctx.bash.sandboxMode)) setSandboxMode(session, spec.sandbox);
	...
	*/
	pinInitialPermission(session) {
		...
		if (sandbox === void 0) setSandboxMode(session, this.ctx.bash.sandboxMode);
		if (approval === void 0) setApprovalPolicy(session, this.ctx.approval.config.policy ?? "ask");
	}
};
//#endregion
export { CUSTOM_PRESET, PERMISSION_SETTINGS_NAMESPACE, PermissionService, PermissionService as default, applyKnobEvent, effectivePermissionPreset };
