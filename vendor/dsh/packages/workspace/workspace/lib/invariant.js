...
const install = Object.assign((ctx, fail) => {
	ctx.on("domain/changed", (change) => {
		...
		if (change.operation === "deleted") {
			if (ctx.workspace.get(WorkspaceId(change.key)) !== void 0) fail(`workspace record '${change.key}' was deleted while the registry cache still publishes it — some write path bypassed ctx.workspace`);
			return;
		}
		if (ctx.workspace.get(WorkspaceId(change.key)) === void 0) fail(`workspace record '${change.key}' landed durably but the registry cache holds no entity for it — the cache and the domain table have diverged`);
	});
}, { inject: ["workspace"] });
/**
