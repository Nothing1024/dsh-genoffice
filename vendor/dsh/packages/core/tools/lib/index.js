...
var ToolRegistry = class extends Service {
	...
	[TOOL_REGISTRY_SCHEDULER] = {
		...
		finish: (exec, result) => {
			return this.finishScheduledExecution(exec, result);
		}
	};
	...
	constructor(ctx, config = {}) {
		...
		this.codeTransport = this.mode === "native" ? void 0 : createRunCodeTool(this, {
		...
		if (this.mode !== "native") ctx.systemPrompt.section({
			name: "tools:sdk",
			order: 150,
			text: (context) => {
	...
	*/
	requireCodeRuntime() {
		...
		if (!runtime) throw new Error(`dsh-tools: mode "${this.mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @deepseek-ai/dsh-code-runtime-worker) or set tools mode to "native"`);
		if (runtime.language !== "typescript") throw new Error(`dsh-tools: mode "${this.mode}" generates a TypeScript SDK, but the loaded code runtime's language is "${runtime.language}"`);
		return runtime;
	}
	/**
	...
	*/
	register(definition) {
		...
		if (this.codeTransport !== void 0 && name === "run_code") throw new Error(`tool name "${RUN_CODE_NAME}" is reserved for the Code Mode presentation transport and cannot be registered or shadowed`);
	...
	*/
	restrict(filter) {
		...
		if (this.codeTransport !== void 0 && [...allow ?? [], ...deny ?? []].includes("run_code")) throw new Error(`tools.restrict() cannot name reserved Code Mode presentation transport "${RUN_CODE_NAME}"; restrict end-capability tools instead`);
	...
	*/
	...
	guardReason(exec) {
		...
		return exec.agent === void 0 ? void 0 : this.layers.peek(exec.agent)?.guardReason(exec);
	}
	/**
	...
	*/
	view(scope) {
		...
		if (this.codeTransport !== void 0) visible.set(RUN_CODE_NAME, this.codeTransport);
		return {
			visible,
			knownNames,
			restrictableNames
...
//#endregion
export { CodeRunFailedError, JsonSchemaError, RUN_CODE_NAME, TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH, TOOL_REGISTRY_SCHEDULER, ToolArgsError, ToolNotFoundError, ToolOutputError, ToolRegistry, ToolRegistry as default, assertObjectJsonSchema, assertSupportedJsonSchema, defineContentToolFixture, defineTool, jsonSchemaToTs, parameterSchemaSpecToJsonSchema, renderToolsSdk, validateArgs, validateJsonSchemaValue, valueSchemaSpecToJsonSchema };
