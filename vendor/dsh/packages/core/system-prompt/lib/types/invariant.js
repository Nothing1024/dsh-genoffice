...
function validateAssembly(assembly, fail) {
    ...
    for (const section of assembly.sections) {
        ...
        if (sectionNames.has(section.name))
            fail(`assembled section name ${JSON.stringify(section.name)} is duplicated`);
        sectionNames.add(section.name);
        if (typeof section.text !== 'string')
            fail(`assembled section ${JSON.stringify(section.name)} text must be a string`);
    }
    ...
    for (const context of assembly.contexts) {
        ...
        if (contextNames.has(context.name))
            fail(`assembled context name ${JSON.stringify(context.name)} is duplicated`);
        contextNames.add(context.name);
        if (typeof context.text !== 'string')
            fail(`assembled context ${JSON.stringify(context.name)} text must be a string`);
    }
    ...
    for (const [name, value] of Object.entries(assembly.variables)) {
        if (!VARIABLE_NAME.test(name))
            fail(`assembled variable name ${JSON.stringify(name)} is invalid`);
