...
var PromptLayer = class {
	...
	*/
	...
	isEmpty() {
		return this.sections.isEmpty() && this.contexts.isEmpty() && this.toolProviders.isEmpty() && this.variables.isEmpty();
	}
};
...
var SystemPrompt = class extends Service {
	...
	*/
	async assemble(context = {}) {
		...
		for (const [name, provider] of scopedVariables?.entries() ?? []) variables[name] = provider(context);
		const sectionByName = this.layers.merge(scope, (layer) => layer.sections);
		const contextByName = this.layers.merge(scope, (layer) => layer.contexts);
		const providers = [...this.layers.global.toolProviders.values(), ...this.layers.peek(scope)?.toolProviders.values() ?? []];
		const collected = [];
		...
		const assembly = {
			sections: [...sectionByName.values()].sort((a, b) => a.order - b.order).map((section) => ({
				name: section.name,
				text: typeof section.text === "function" ? section.text(context) : section.text
			})),
			contexts: [...contextByName.values()].sort((a, b) => a.order - b.order).map((entry) => ({
				name: entry.name,
...
//#endregion
export { SystemPrompt, SystemPrompt as default, TOOL_ORDER_REST, joinContextSections, renderContextSections, renderContextSnapshot, renderPrompt };
