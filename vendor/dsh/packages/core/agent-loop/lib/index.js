...
import { TOOL_ABORTED_BEFORE_DISPATCH, TOOL_REGISTRY_SCHEDULER } from "@deepseek-ai/dsh-tools";
...
async function runGroup(ctx, turn, step, group, mode, signal, acceptContext) {
	...
	const commitReady = async () => {
		while (committed < group.length) {
			...
			const result = slot.needsPost ? await ctx.tools[TOOL_REGISTRY_SCHEDULER].finalize(slot.exec, slot.result) : ctx.tools[TOOL_REGISTRY_SCHEDULER].finish(slot.exec, slot.result);
...
var ReactLoopAgent = class {
	...
	send(message, target, wakeup) {
		const resolvedTarget = wakeup && this.phase.kind !== "idle" && this.phase.abort.signal.aborted ? "next-turn" : target;
	...
	async kick() {
		try {
			...
			if (this.phase.kind === "running") this.setPhase({
				kind: "idle",
				lastTurn: this.phase.turn
			});
		}
	}
...
var FactoryOwnership = class {
	...
	trackWrapper(task) {
		this.trackStartup(task.then(() => void 0, () => void 0));
	}
	/** Resolve `task`, or stop waiting when factory teardown begins. */
	async waitWhileActive(task) {
...
//#endregion
export { AgentLoop, AgentLoop as default, CONFIGURED_AGENT_IDENTITIES_KEY, DEFAULT_MAX_PARALLEL_TOOL_CALLS };
