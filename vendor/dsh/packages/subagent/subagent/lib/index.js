...
*/
function applyChildComposition(childCtx, composition) {
...
async function listChildren(ctx, parentSessionId, signal) {
	...
	if (query === void 0) throw new SubagentError("listing subagents requires session query (load a dsh-session-query backend)", "SUBAGENT_CONTROL_SESSION_QUERY_UNAVAILABLE");
...
async function inspectChild(query, queryRuntime, parentSessionId, candidate, hasChildren, signal) {
	...
	try {
		...
		const descriptorSeqs = records.filter((record) => record.seq >= seedLength && record.type === "subagent/descriptor").map((record) => record.seq);
		if (descriptorSeqs.length === 0) return void 0;
		if (descriptorSeqs.length > 1) return {
			kind: "diagnostic",
			id: childId,
			reason: "corrupt"
		};
		const seq = descriptorSeqs[0];
		const window = await runListingQuery(() => query.readEvent({
			sessionId: childId,
			seq
		}, signal), signal);
		queryRuntime.assertSessionHeadersCompatible(window.session, candidate.header);
		if (window.session.parentSession !== parentSessionId || window.target.type !== "subagent/descriptor") return {
			kind: "diagnostic",
			id: childId,
			reason: "corrupt"
		};
		...
		if (descriptor.mode === "one-shot") return {
			kind: "child",
			id: childId,
...
function perChildDiagnosticReason(error, SessionQueryError) {
	if (!(error instanceof SessionQueryError)) return void 0;
	switch (error.code) {
		case "SESSION_QUERY_CORRUPT_SESSION": return "corrupt";
		case "SESSION_QUERY_SESSION_NOT_FOUND":
		case "SESSION_QUERY_EVENT_NOT_FOUND":
		case "SESSION_QUERY_PERSISTENCE_FAILED": return "unavailable";
		case "SESSION_QUERY_INVALID_SURFACE":
		case "SESSION_QUERY_SOURCE_CONFLICT": return "corrupt";
		default: return;
	}
}
/**
...
//#endregion
export { NO_START_CAPABILITIES, SUBAGENT_DESCRIPTOR_VERSION, SubagentDepthError, SubagentError, SubagentRunId, SubagentService, SubagentService as default, applyChildComposition, assertPositiveFinite, assertSubagentMaxDepth, assertUsableCwd, childSessionMeta, delegationDepthOf, foldSubagentDescriptor, resolveChildAgentOptions, resolveChildCwd, resolveChildDepth, seedDescriptorTurn, settleRun, settleRunResult, snapshotSubagentDescriptor, subprocessRunHandle, validateConfiguredCwd };
