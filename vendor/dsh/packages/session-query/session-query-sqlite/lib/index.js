...
import SessionQueryService, { SESSION_QUERY_DEFAULT_PERSISTED_INSPECT_CONCURRENCY, SESSION_QUERY_READ_WINDOW_MAX, SessionQueryError, SessionSearchCursor, assertSessionHeadersCompatible, buildSessionEventSearchDocuments, materializeSessionEventResultFilters, materializeSessionResultFilters } from "@deepseek-ai/dsh-session-query";
import { mkdir, open } from "node:fs/promises";
import { dirname, resolve } from "node:path";
//#region lib/types/schema.js
/** SQLite schema for the disposable session full-text read model. */
/** Current derived-index schema version. Incompatible versions reset in place. */
const SESSION_QUERY_SQLITE_SCHEMA_VERSION = 7;
...
async function openSearchDatabase(path, journalMode) {
	...
	try {
		...
		if (applicationId === 1146308689) {
			...
			if (version !== 7) resetDerivedSchema(db, userTables);
		}
...
var SessionQuerySqlite = class extends SessionQueryService {
	static inject = ["sessions"];
	static Config = z.object({
		path: z.string().required(),
		openAt: z.union(["startup", "first-search"]).default("startup"),
		journalMode: z.union([
			"wal",
			"delete",
			"truncate",
 ...
 /* v8 ignore next 5 -- ROLLBACK failure requires a SQLite double fault; the original failure remains actionable. */
	...
	_replacePersistedSession(entry, revision, generation) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(entry.header.id, entry.header.version, entry.header.createdAt, entry.header.cwd ?? null, entry.header.parentSession ?? null, entry.header.seedLength ?? null, entry.header.delegationDepth ?? null, revision, generation);
	...
	_replaceLiveSession(entry, generation, persisted) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(entry.header.id, entry.header.version, entry.header.createdAt, entry.header.cwd ?? null, entry.header.parentSession ?? null, entry.header.seedLength ?? null, entry.header.delegationDepth ?? null, entry.fingerprint, persisted ? 1 : 0, generation);
		const insert = db.prepare(`
...
function sameHeader(a, b) {
	return a.version === b.version && a.id === b.id && a.createdAt === b.createdAt && a.cwd === b.cwd && a.parentSession === b.parentSession && a.seedLength === b.seedLength && (a.delegationDepth ?? 0) === (b.delegationDepth ?? 0);
}
...
function resolveConfig(config) {
	...
	if (!["startup", "first-search"].includes(resolved.openAt)) throw invalidConfig("openAt is not supported");
...
//#endregion
export { SESSION_QUERY_SQLITE_APPLICATION_ID, SESSION_QUERY_SQLITE_DEFAULT_LIMIT, SESSION_QUERY_SQLITE_MAX_LIMIT, SESSION_QUERY_SQLITE_PATH_KEY, SESSION_QUERY_SQLITE_SCHEMA_VERSION, SESSION_QUERY_SQLITE_SNIPPET_CHARS, SessionQuerySqlite, SessionQuerySqlite as default };
