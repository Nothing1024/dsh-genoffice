...
function listUserTables(db) {
    const rows = db.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name NOT GLOB 'sqlite_*' ORDER BY name").all();
...
function assertDerivedUserTables(path, userTables) {
    ...
    if (unknownTables.length > 0) {
        throw new Error(`session-search database at "${path}" has unrecognized user tables: ${unknownTables.join(', ')}`);
    }
}
