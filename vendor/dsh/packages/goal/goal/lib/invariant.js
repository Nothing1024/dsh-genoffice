...
function decodeBlockReason(value) {
	if (!isRecord(value) || Object.keys(value).sort().join(",") !== "code,message") throw new Error("goal change goal.blockedReason has an invalid shape");
...
function decodeSnapshot(value) {
	...
	if (Object.keys(value).sort().join(",") !== expectedKeys) throw new Error("goal change goal has an invalid shape");
	return {
		id: GoalId(value["id"]),
...
function decodeRef(value) {
	if (!isRecord(value) || Object.keys(value).sort().join(",") !== "id,revision") throw new Error("goal clear tombstone has an invalid shape");
...
function decodeGoalChange(value) {
	...
	if (value["operation"] === "clear") {
		if (Object.keys(value).sort().join(",") !== [
			...
			"operation",
			"version"
		].sort().join(",")) throw new Error("goal clear change has an invalid shape");
		return {
			kind: "goal/change",
			version: 1,
			operation: "clear",
	...
	if (Object.keys(value).sort().join(",") !== [
		...
		"updatedAt",
		"version"
	].sort().join(",")) throw new Error("goal snapshot change has an invalid shape");
...
const install = Object.assign((ctx, fail) => {
	...
	const stateFor = (session) => {
		return states.get(session) ?? seed(session);
	};
