...
export class CordisCatalogProjector {
    ...
    constructor(face, sourceDeclarations, policy) {
        ...
        this.policy = policy;
        if (face.face !== 'host')
            throw new Error(`cordis catalog requires the host face, received ${face.face}`);
        ...
        for (const packageModel of this.face.packages) {
            for (const service of packageModel.services) {
                ...
                if (declaration.kind !== 'class'
                    ...
                    || declaration.location.file !== service.location.file)
                    continue;
                ...
                for (const memberId of service.members) {
                    ...
                    if (member.kind !== 'method' || member.name.startsWith('['))
                        continue;
    ...
    runtimeTypes(services) {
        ...
        for (const declaration of this.sourceDeclarations) {
            if (declaration.face !== 'host' || declaration.kind === 'enum'
                || !/^packages\/[^/]+\/[^/]+\/src\/[^/]+\.ts$/.test(declaration.location.file))
                continue;
 ...
 */
export function projectCordisCatalog(scanRoot, policy) {
    ...
    if (face === undefined)
        throw new Error('gen-cordis-catalog: Typert produced no host face');
 ...
 */
export function renderEvents(events, policy) {
    const lines = [
        ...BANNER,
 ...
 * @returns the complete generated Markdown document.
 */
export function renderServices(services, policy) {
    const lines = [
        ...BANNER,
    ...
    for (const s of services)
        lines.push(...renderService(s, policy.linkedTypePages));
