...
async function readCached(directory, specifier) {
    ...
    if (typeof parsed !== 'object' || parsed === null || typeof parsed.specifier !== 'string') {
    ...
    if (!packageStat.isDirectory())
        throw new Error(`repository cache package is not a directory: ${packageDirectory}`);
...
export class RepositoryCache {
    ...
    resolve(specifier) {
        ...
        const task = this.resolveUncached(specifier).finally(() => {
            if (this.tasks.get(specifier) === task)
                this.tasks.delete(specifier);
        });
    ...
    async resolveUncached(specifier) {
        ...
        try {
            ...
            if (!packageStat.isDirectory())
                throw new Error(`installed repository is not a directory: ${packageDirectory}`);
